//! Type conversions between `crow_text` types and `lsp_types`.
//!
//! Both `crow_text::Position` and `lsp_types::Position` are zero-based
//! line/character pairs, so the mapping is straightforward.

use crow_text::{Position, Range};

/// Converts a `crow_text::Position` to an `lsp_types::Position`.
...
/// Converts an `lsp_types::Position` to a `crow_text::Position`.
pub fn lsp_to_position(pos: lsp_types::Position) -> Position {
    Position::new(pos.line, pos.character)
}

/// Converts a `crow_text::Range` to an `lsp_types::Range`.
...
/// Converts an `lsp_types::Range` to a `crow_text::Range`.
