    ...
    pub fn parse(raw: &str) -> Self {
        ...
        if let Some((scheme, rest)) = raw.split_once("://") {
            let (authority, path) = rest
                .split_once('/')
                .map(|(a, p)| (a.to_owned(), format!("/{p}")))
                .unwrap_or((String::new(), rest.to_owned()));
            return Self {
                scheme: scheme.into(),
                authority,
                path,
...
impl<T> EventEmitter<T> {
    ...
    pub fn fire(&self, event: &T) {
        for (_, cb) in self.listeners.read().expect("lock").iter() {
            cb(event);
        }
    }
}
...
pub struct Diagnostic {
    pub range: crow_extensions::protocol::Range,
    pub message: String,
...
impl DiagnosticCollection {
    ...
    pub fn delete(&self, uri: &str) {
        self.entries.write().expect("lock").remove(uri);
    }
    pub fn clear(&self) {
        self.entries.write().expect("lock").clear();
    }
    pub fn has(&self, uri: &str) -> bool {
        self.entries.read().expect("lock").contains_key(uri)
    }
}
...
pub type AuthenticationProviderHandler =
    Arc<dyn Fn(&str, Value) -> anyhow::Result<Value> + Send + Sync>;
...
impl AuthenticationProviderRegistry {
    ...
    pub fn unregister(&self, id: &str) {
        self.providers.write().expect("lock").remove(id);
    }
...
impl Default for AuthenticationProviderRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
pub type NotebookSerializerHandler =
    Arc<dyn Fn(&str, &[u8]) -> anyhow::Result<Value> + Send + Sync>;
...
impl NotebookSerializerRegistry {
    ...
    pub fn register(&self, notebook_type: &str, handler: NotebookSerializerHandler) {
        self.serializers
            .write()
            .expect("lock")
            .insert(notebook_type.to_owned(), handler);
    }
...
impl Default for NotebookSerializerRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
impl CustomEditorProviderRegistry {
    ...
    pub fn register(&self, view_type: &str, handler: CustomEditorHandler) {
        self.providers
            .write()
            .expect("lock")
            .insert(view_type.to_owned(), handler);
    }
...
impl Default for CustomEditorProviderRegistry {
    fn default() -> Self {
        Self::new()
    }
}
