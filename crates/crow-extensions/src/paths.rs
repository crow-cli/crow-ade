//! Standard filesystem paths for `Crow` extension storage.
//!
//! Ported from `src-tauri/src/commands/extension_platform.rs`.

use std::path::PathBuf;

/// Root `Crow` data directory (`~/.crow`).
pub fn crow_data_dir() -> PathBuf {
    dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
        .join(".crow")
}

/// User-installed extensions directory (`~/.crow/extensions`).
pub fn user_extensions_dir() -> PathBuf {
    let dir = crow_data_dir().join("extensions");
...
pub fn resolve_node_runtime() -> Result<NodeRuntime, String> {
    ...
    Err("Node runtime not found. Install Node.js (>=18) or set CROW_NODE_BINARY.".into())
}
