...
//! `.code-workspace` (or `.crow-workspace`) file.
//!
...
/// Parsed `.code-workspace` / `.crow-workspace` file.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
...
/// Parse a `.code-workspace` or `.crow-workspace` JSON file.
...
pub fn is_workspace_file(path: &Path) -> bool {
    ...
    matches!(ext, "code-workspace" | "crow-workspace")
}
...
mod tests {
    ...
    #[test]
    fn save_and_reload() {
        let tmp = std::env::temp_dir().join("crow-ws-test.code-workspace");
    ...
    fn is_workspace_file_detection() {
        ...
        assert!(is_workspace_file(Path::new("project.crow-workspace")));
    ...
    fn save_workspace_file_fn() {
        let tmp = std::env::temp_dir().join("crow-save-ws-test.crow-workspace");
        let config = WorkspaceConfig {
