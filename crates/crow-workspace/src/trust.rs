...
const TRUST_FILE_NAME: &str = ".crow-trusted-workspaces.json";
...
fn config_dir() -> PathBuf {
    if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".config").join("crow")
    } else if let Ok(home) = std::env::var("USERPROFILE") {
        PathBuf::from(home).join(".config").join("crow")
    } else {
        PathBuf::from(".config").join("crow")
    }
}
...
mod tests {
    ...
    fn temp_trust_dir(name: &str) -> PathBuf {
        let tmp = std::env::temp_dir().join(format!("crow-trust-{name}"));
