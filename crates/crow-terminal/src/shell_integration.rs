...
if [[ -z "$__crow_installed" ]]; then
    __crow_installed=1
    trap '__crow_preexec "$BASH_COMMAND"' DEBUG
    PROMPT_COMMAND="__crow_prompt_command${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
fi
"#;
...
if [[ -z "$__crow_installed" ]]; then
    ...
    add-zsh-hook preexec __crow_preexec
    add-zsh-hook precmd __crow_precmd
fi
"#;

const FISH_INTEGRATION: &str = r#"
function __crow_preexec --on-event fish_preexec
    printf '\e]633;C\a'
    printf '\e]633;E;%s\a' "$argv[1]"
end
function __crow_postexec --on-event fish_postexec
...
if (-not $__crow_installed) {
    $__crow_installed = $true
    $__crow_original_prompt = $function:prompt
    function prompt {
        $exitCode = $LASTEXITCODE
        [Console]::Write("`e]633;D;$exitCode`a")
        [Console]::Write("`e]633;A`a")
        [Console]::Write("`e]633;P;Cwd=$PWD`a")
        & $__crow_original_prompt
    }
