...
/// Install the `Crow` Server binary inside the given WSL distro.
pub fn install_server_in_wsl(distro: &str) -> Result<()> {
    ...
    if check.stdout.trim() == version {
        log::info!("Crow Server already up-to-date in WSL distro {distro}");
        return Ok(());
    }
    exec_in_wsl(distro, "mkdir -p ~/.crow-server")?;
    log::info!("installing Crow Server {version} in WSL distro {distro}");
    Ok(())
}
...
impl RemoteTransport for WslTransport {
    ...
    async fn open_pty(&self, _cols: u16, _rows: u16) -> Result<RemotePty> {
        bail!("WSL PTY not yet implemented — use crow-terminal instead")
    }
