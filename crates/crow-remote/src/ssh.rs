...
impl SshConnection {
    ...
    pub async fn install_server(&mut self) -> Result<()> {
        ...
        let check = self
            .exec_command("~/.crow-server/crow-server --version 2>/dev/null || echo missing")
            .await?;

        if check.stdout.trim() == version {
            log::info!("Crow Server {version} already installed");
            self.state = ConnectionState::Connected;
            return Ok(());
        }
        ...
        self.exec_command("mkdir -p ~/.crow-server").await?;
        log::info!("installing Crow Server {version} on remote");
        self.state = ConnectionState::Connected;
        Ok(())
    }
