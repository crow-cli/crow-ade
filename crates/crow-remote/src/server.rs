//! `Crow` Server — runs on the remote machine.
//!
...
impl CrowServer {
    ...
    pub async fn start_tcp(self: Arc<Self>, port: u16) -> Result<()> {
        ...
        log::info!("Crow Server listening on port {port}");
        loop {
...
mod tests {
    ...
    #[test]
    fn base64_roundtrip() {
        let original = b"Hello, Crow remote server!";
    ...
    async fn server_fs_mkdir_and_stat() {
        let server = CrowServer::new();
        let tmp = std::env::temp_dir().join("crow_test_mkdir");
