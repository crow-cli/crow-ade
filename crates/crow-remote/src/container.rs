...
/// List all Docker containers managed by `Crow`.
pub async fn list_containers() -> Result<Vec<ContainerInfo>> {
    ...
    let opts = bollard::container::ListContainersOptions::<String> {
        ...
        filters: {
            ...
            f.insert("name".to_string(), vec!["crow-".to_string()]);
            f
        },
        ..Default::default()
    };
...
/// Install the `Crow` Server inside a running container.
pub async fn install_server_in_container(container_id: &str) -> Result<()> {
    ...
    let check = exec_in_container(
        &docker,
        container_id,
        "~/.crow-server/crow-server --version 2>/dev/null || echo missing",
        "root",
    )
    .await?;

    if check.stdout.trim() == version {
        log::info!("Crow Server already up-to-date in container {container_id}");
        return Ok(());
    }

    exec_in_container(&docker, container_id, "mkdir -p ~/.crow-server", "root").await?;
    log::info!("installing Crow Server {version} in container {container_id}");
    Ok(())
}
...
impl ContainerTransport {
    ...
    async fn build_image_inner(
        ...
        let tag = format!("crow-devcontainer:{:x}", fxhash(context_path));
    ...
    async fn create_container_inner(
        ...
        let name = format!("crow-{:x}", fxhash(workspace_path));
