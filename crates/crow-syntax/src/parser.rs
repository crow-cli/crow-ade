...
/// Convert a [`crow_text::EditOperation`] + [`crow_text::Buffer`] into a
/// tree-sitter [`InputEdit`].
///
/// The buffer should reflect the state **before** the edit was applied.
#[must_use]
pub fn to_input_edit(edit: &crow_text::EditOperation, buffer: &crow_text::Buffer) -> InputEdit {
...
mod tests {
    use super::*;
    use crate::language::Language as CrowLanguage;

    fn make_rust_lang() -> CrowLanguage {
        CrowLanguage {
            name: "rust".into(),
    ...
    fn to_input_edit_insert() {
        let buffer = crow_text::Buffer::from_str("fn main() {}");
        let edit_op = crow_text::EditOperation::insert(
            crow_text::Position::new(0, 11),
            " let x = 1;".into(),
        );
    ...
    fn to_input_edit_delete() {
        let buffer = crow_text::Buffer::from_str("fn main() { let x = 1; }");
        let edit_op = crow_text::EditOperation::delete(crow_text::Range::new(
            crow_text::Position::new(0, 11),
            crow_text::Position::new(0, 23),
        ));
