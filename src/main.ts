...
(window as any).__crow_openFolder = crowOpenFolder;
...
async function boot() {
	...
	// Crow Rust bridge initialization — make services available before workbench creation
	if ((globalThis as any).__CROW_TAURI__) {
		const {
			CrowEditorBridge,
			CrowSyntaxService,
			CrowGitService,
			...
			CrowSettingsService,
			CrowThemeService,
			CrowExtensionService,
			CrowKeymapService,
			CrowFileSystemProvider
		} = await import('./vs/platform/crow/common/crowServices.js');

		(globalThis as any).__CROW_SERVICES__ = {
			editor: CrowEditorBridge.getInstance(),
			syntax: new CrowSyntaxService(),
			git: new CrowGitService(),
			search: new CrowSearchService(),
			settings: new CrowSettingsService(),
			theme: new CrowThemeService(),
			extensions: new CrowExtensionService(),
			keymap: new CrowKeymapService(),
			fileSystem: new CrowFileSystemProvider()
		};
...
function setupTauriExternalOpener() {
	import('@tauri-apps/plugin-shell')
		.then(shell => {
			(window as any).__crow_shellOpen = (url: string) => {
...
function setupWindowsEditorNewlineKeybindings() {
	...
	window.addEventListener('keydown', event => {
		...
		const commandService = (
			window as { __crow_commandService?: { executeCommand(commandId: string): Promise<unknown> } }
		).__crow_commandService;
		if (!commandService) {
			return;
		}
...
function setupMenuActions() {
	...
	(window as any).__crow_menu_action = async (menuId: string) => {
		...
		if (menuId === 'find') {
			const editorService = (window as any).__crow_editorService;
			if (editorService?.activeEditor?.typeId === 'workbench.editors.webviewInput') {
				try {
					await (window as any).__crow_commandService?.executeCommand('editor.action.webvieweditor.showFind');
				} catch {}
				return;
			}
		}
		...
		try {
			const event = new CustomEvent('crow-command', { detail: { commandId } });
	...
	window.addEventListener('crow-native-menu', ((e: CustomEvent) => {
		const menuId = e.detail;
		if (typeof menuId === 'string' && (window as any).__crow_menu_action) {
			(window as any).__crow_menu_action(menuId);
		}
	}) as EventListener);

	// Listen for command execution via keyboard shortcuts forwarded from native menu
	window.addEventListener('crow-command', async (e: any) => {
		...
		try {
			const commandService = (window as any).__crow_commandService;
			if (commandService) {
