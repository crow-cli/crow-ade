...
const crowTranslations: Record<string, Translations> = {
	'zh-cn': {
		...
		'crow.remote.refresh': '刷新远程资源管理器',
		'crow.remote.openExplorer': '远程资源管理器',
...
export async function loadNlsMessages(): Promise<void> {
	...
	console.log('[Crow NLS] locale from localStorage:', locale);
	...
	console.log('[Crow NLS] extensionId from localStorage:', extensionId);

	if (!extensionId) {
		extensionId = await detectInstalledLanguagePack(locale);
		if (extensionId) {
			localStorage.setItem('vscode.nls.languagePackExtensionId', extensionId);
			console.log('[Crow NLS] auto-detected language pack:', extensionId);
		} else {
			console.warn('[Crow NLS] No language pack found for locale:', locale);
			return;
		}
	}
	...
	try {
		...
		if (!translations) {
			console.warn('[Crow NLS] No translations found for', extensionId);
			return;
		}

		// Merge Crow-specific translations for strings not in the VS Code language pack
		const crowExtra = crowTranslations[locale.toLowerCase()];
		if (crowExtra) {
			for (const [key, val] of Object.entries(crowExtra)) {
		...
		if (indexRes.ok) {
			...
			if (contentType.includes('json')) {
				...
				if (nlsEntries.length > 0) {
					...
					console.log(`[Crow NLS] Loaded ${nlsEntries.length} translations for ${locale} (indexed mode)`);
					return;
				}
			}
		}
		...
		(globalThis as any)._VSCODE_NLS_LANGUAGE = locale;
		console.log(`[Crow NLS] Loaded ${Object.keys(translations).length} translations for ${locale} (key mode)`);
	} catch (e) {
		console.warn('[Crow NLS] Failed to load translations:', e);
	}
}
...
async function loadFromDisk(extensionId: string): Promise<Translations | null> {
	try {
		...
		const path = `${homedir}/.crow/extensions/${extensionId}/translations/main.i18n.json`;
		const raw = await invoke<string>('read_file', { path });
		const result = parseBundle(raw);
		if (result) {
			console.log(`[Crow NLS] Loaded translations from disk: ${path}`);
		}
		return result;
	} catch {
		return null;
	...
	for (const metaUrl of urls) {
		try {
			...
			if (res.ok) {
				...
				if (result) {
					console.log(`[Crow NLS] Loaded translations from gallery`);
					return result;
				}
			}
		} catch {
...
async function detectInstalledLanguagePack(locale: string): Promise<string | null> {
	...
	try {
		...
		const path = `${homedir}/.crow/extensions/${knownId}/translations/main.i18n.json`;
