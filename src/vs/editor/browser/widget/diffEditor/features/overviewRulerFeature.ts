...
export class OverviewRulerFeature extends Disposable {
	...
	constructor(
		...
		const currentColorTheme = observableFromEvent(this._themeService.onDidColorThemeChange, () =>
		...
		const currentColors = derived(reader => {
			...
			const removeColor =
				theme.getColor(diffOverviewRulerRemoved) || (theme.getColor(diffRemoved) || defaultRemoveColor).transparent(2);
			...
			autorunWithStore((reader, store) => {
				...
				store.add(
					autorun(reader => {
						...
						if (layoutInfo) {
							const freeSpace =
								OverviewRulerFeature.ENTIRE_DIFF_OVERVIEW_WIDTH - 2 * OverviewRulerFeature.ONE_OVERVIEW_WIDTH;
