...
export interface IFormattingEditProviderSelector {
	<T extends DocumentFormattingEditProvider | DocumentRangeFormattingEditProvider>(
		formatter: T[],
		document: ITextModel,
		mode: FormattingMode,
		kind: FormattingKind
...
export abstract class FormattingConflicts {
	...
	static async select<T extends DocumentFormattingEditProvider | DocumentRangeFormattingEditProvider>(
		formatter: T[],
		document: ITextModel,
		mode: FormattingMode,
		kind: FormattingKind
...
export async function formatDocumentRangesWithSelectedProvider(
	...
	const { documentRangeFormattingEditProvider: documentRangeFormattingEditProviderRegistry } =
...
export async function formatDocumentRangesWithProvider(
	...
	const computeEdits = async (range: Range) => {
		...
		const result =
			(await provider.provideDocumentRangeFormattingEdits(model, range, model.getFormattingOptions(), cts.token)) || [];
...
export async function getDocumentFormattingEditsUntilResult(
	...
	for (const provider of providers) {
		const rawEdits = await Promise.resolve(provider.provideDocumentFormattingEdits(model, options, token)).catch(
...
export async function getDocumentFormattingEditsWithSelectedProvider(
	...
	if (selected) {
		const rawEdits = await Promise.resolve(
			selected.provideDocumentFormattingEdits(model, model.getOptions(), token)
		).catch(onUnexpectedExternalError);
		return await workerService.computeMoreMinimalEdits(model.uri, rawEdits);
	}
	return undefined;
...
CommandsRegistry.registerCommand('_executeFormatRangeProvider', async function (accessor, ...args) {
	...
	try {
		return getDocumentRangeFormattingEditsUntilResult(
			...
			Range.lift(range),
			ensureFormattingOptions(options, reference),
