...
export class TextModelChangeRecorder extends Disposable {
	...
	constructor(
		...
		this._structuredLogger = this._register(
			this._instantiationService.createInstance(
				StructuredLogger.cast<IRecordableEditorLogEntry & IDocumentEventDataSetChangeReason>(),
			...
			autorun(reader => {
				...
				reader.store.add(
					this._editor.onDidChangeModelContent(e => {
						...
						logger.trace(
							'onDidChangeModelContent: ' + e.detailedReasons.map(r => r.toKey(Number.MAX_VALUE)).join(', ')
						);
					})
				);
			})
