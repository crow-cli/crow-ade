...
export class GhostText {
	...
	renderForScreenReader(lineText: string): string {
		...
		const text = new TextEdit([
			...this.parts.map(
				p =>
					new TextReplacement(Range.fromPositions(new Position(1, p.column)), p.lines.map(line => line.line).join('\n'))
			)
...
export class GhostTextReplacement {
	...
	render(documentText: string, debug: boolean = false): string {
		...
		if (debug) {
			...
			return new TextEdit([new TextReplacement(replaceRange, this.newLines.join('\n'))]).applyToString(documentText);
		}
	}
