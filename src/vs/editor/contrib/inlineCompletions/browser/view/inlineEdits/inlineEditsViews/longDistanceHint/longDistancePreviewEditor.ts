...
export class LongDistancePreviewEditor extends Disposable {
	...
	constructor(
		...
		this._register(
			this._previewEditorObs.setDecorations(
				derived(reader => {
					const state = this._state.read(reader);
					const decorations = this._editorDecorations.read(reader);
					return (
						(state?.mode === 'original' ? decorations?.originalDecorations : decorations?.modifiedDecorations) ?? []
					);
				})
			)
		);
		...
		this._register(
			autorun(reader => {
				...
				if (cursorPosition) {
					this.previewEditor.setPosition(
						this._previewTextModel.validatePosition(cursorPosition),
	...
	public readonly contentHeight = derived(this, reader => {
		...
		const previewEditorHeight = this._previewEditorObs.observeLineHeightForLine(
			viewState.visibleLineRange.startLineNumber
		);
	...
	private _getHorizontalContentRangeInPreviewEditorToShow(editor: ICodeEditor, reader: IReader) {
		...
		if (jumpToPos) {
			...
			firstCharacterChange =
				state.mode === 'modified' ? diff[0].innerChanges[0].modifiedRange : diff[0].innerChanges[0].originalRange;
		} else {
			return undefined;
		}
		...
		for (const m of diff.diff) {
			if (m.modified.isEmpty || m.original.isEmpty) {
				...
				for (const i of m.innerChanges || []) {
					...
					if (
						m.original.contains(i.originalRange.startLineNumber) &&
						!(hideEmptyInnerDecorations && i.originalRange.isEmpty())
					) {
...
function growUntilVariableBoundaries(textModel: ITextModel, range: Range, maxGrow: number): Range {
	...
	while (
		startColumn > 1 &&
		isVariableNameCharacter(startColumn) &&
		!isWhitespace(startColumn - 1) &&
	...
	while (
		...
		isVariableNameCharacter(endColumn) &&
		!isWhitespace(endColumn + 1) &&
