...
export class JumpToView extends Disposable {
	...
	constructor(
		...
		this._register(
			this._editor.setDecorations(
				derived<IModelDeltaDecoration[]>(reader => {
	...
	private readonly _pos = derived(this, reader => {
		return this._editor.observePosition(
			derived(reader => this._data.read(reader)?.jumpToPosition || null),
			reader.store
		);
	}).flatten();
...
function animateFixedValues<T>(
	values: { value: T; durationMs: number }[],
