...
export class InlineEditsDeletionView extends Disposable implements IInlineEditsView {
	...
	private readonly _editorLayoutInfo = derived(this, reader => {
		...
		const right =
			editorLayout.contentLeft + Math.max(this._editorMaxContentWidthInRange.read(reader), w) - horizontalScrollOffset;
		...
		const codeRect = Rect.fromLeftTopRightBottom(left, selectionTop, right, selectionBottom).withMargin(
			VERTICAL_PADDING,
			HORIZONTAL_PADDING
		);
	...
	private readonly _originalOverlay = n
		.div(
			{
			...
			derived(this, reader => {
				...
				const overlayhider = layoutInfoObs.map(layoutInfo =>
				...
				const separatorWidth = this._uiState
					.map(s =>
				...
				const editorBackground = getEditorBackgroundColor(
					this._uiState.map(s => s?.editorType ?? InlineCompletionEditorType.TextEditor).read(reader)
				);
				return [
					n.div({
	...
	private readonly _nonOverflowView = n
		.div(
			{
