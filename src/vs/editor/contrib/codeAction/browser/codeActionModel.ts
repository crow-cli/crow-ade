...
export class CodeActionModel extends Disposable {
	...
	private _update(): void {
		...
		if (model && this._registry.has(model) && !this._editor.getOption(EditorOption.readOnly)) {
			...
			const oracle = new CodeActionOracle(
				this._editor,
				this._markerService,
				trigger => {
					if (!trigger) {
					...
					const actions = createCancelablePromise(async token => {
						if (
							...
							trigger.trigger.type === CodeActionTriggerType.Invoke &&
							...
							if (foundQuickfix) {
								...
								if (allMarkers.length > 0) {
									...
									const filteredActions = currentActions.filter(
										(action, index, self) => self.findIndex(a => a.action.title === action.action.title) === index
									);
					...
					if (this._state.type === CodeActionsState.Type.Triggered) {
						...
						isManualToAutoTransition =
							this._state.trigger.type === CodeActionTriggerType.Invoke &&
