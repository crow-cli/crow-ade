...
function registerQuickInputCommandAndKeybindingRule(
	rule: PartialExcept<ICommandAndKeybindingRule, 'id' | 'handler'>,
	options: { withAltMod?: boolean; withCtrlMod?: boolean; withCmdMod?: boolean } = {}
) {
...
function getSecondary(
	primary: number,
	secondary: number[],
	options: { withAltMod?: boolean; withCtrlMod?: boolean; withCmdMod?: boolean; withShiftMod?: boolean } = {}
): number[] {
...
function focusHandler(focus: QuickPickFocus, focusOnQuickNatigate?: QuickPickFocus): ICommandHandler {
	return accessor => {
		...
		const currentQuickPick = accessor.get(IQuickInputService).currentQuickInput as
			| IQuickPick<any>
			| IQuickTree<any>
			| undefined;
		if (!currentQuickPick) {
