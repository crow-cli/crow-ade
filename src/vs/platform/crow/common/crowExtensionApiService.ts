...
export const ICrowExtensionApiService = createDecorator<ICrowExtensionApiService>('crowExtensionApiService');

export interface ICrowExtensionApiService {
...
export class CrowExtensionApiService implements ICrowExtensionApiService {
	...
	async callCommand(command: string, args?: unknown): Promise<ExtCommandResult> {
		...
		if (!namespace || !action) {
			throw new Error(`[CrowExtensionApiService] malformed command id: "${command}"`);
		}
...
registerSingleton(ICrowExtensionApiService, CrowExtensionApiService, InstantiationType.Delayed);
