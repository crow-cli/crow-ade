/*---------------------------------------------------------------------------------------------
 *  Crow — Remote development service.
 *  Thin TypeScript wrapper over the Tauri commands backed by the
 *  `crow-remote` crate (SSH, WSL, Dev Containers, Codespaces).
 *
...
import { invoke, isTauri } from '../../../crow-bridge.js';
...
export const ICrowRemoteService = createDecorator<ICrowRemoteService>('crowRemoteService');

export interface ICrowRemoteService extends CrowRemoteService {
	readonly _serviceBrand: undefined;
}

export class CrowRemoteService {
...
let _instance: CrowRemoteService | null = null;

export function getCrowRemoteService(): CrowRemoteService {
	if (!_instance) {
		_instance = new CrowRemoteService();
	}
	return _instance;
}

registerSingleton(ICrowRemoteService, CrowRemoteService, InstantiationType.Delayed);
