...
function isIExtension(extension: unknown): extension is IExtension {
	return (
		(extension as IExtension).type === ExtensionType.User || (extension as IExtension).type === ExtensionType.System
	);
}
...
export class AllowedExtensionsService extends Disposable implements IAllowedExtensionsService {
	...
	private getAllowedExtensionsValue(): AllowedExtensionsConfigValueType | undefined {
		const value = this.configurationService.getValue<AllowedExtensionsConfigValueType | undefined>(
			AllowedExtensionsConfigKey
		);
	...
	isAllowed(
		...
		const settingsCommandLink = createCommandUri('workbench.action.openSettings', {
		...
		const extensionReason = new MarkdownString(
			nls.localize('specific extension not allowed', 'it is not in the [allowed list]({0})', settingsCommandLink)
		);
		...
		const publisherKey =
			publisherDisplayName && this.publisherOrgs.includes(publisherDisplayName) ? publisherDisplayName : publisher;
