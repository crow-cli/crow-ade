...
export class ExtensionManagementCLI {
	...
	public async installExtensions(
		...
		try {
			...
			for (const extension of builtinExtensions) {
				if (extension instanceof URI) {
					installVSIXInfos.push({
						vsix: extension,
						installOptions: { ...installOptions, isBuiltin: true, donotIncludePackAndDependencies: true }
					});
				} else {
					const [id, version] = getIdAndVersion(extension);
			...
			this.logger.error(
				localize('error while installing extensions', 'Error while installing extensions: {0}', getErrorMessage(error))
			);
			throw error;
		}
	...
	public async updateExtensions(profileLocation?: URI): Promise<void> {
		...
		for (const newVersion of availableVersions) {
			for (const oldVersion of installedExtensions) {
				if (
					areSameExtensions(oldVersion.identifier, newVersion.identifier) &&
	...
	private async installGalleryExtensions(
		...
		await Promise.all(
			installExtensionInfos.map(async ({ id, version, installOptions }) => {
				...
				if (installedExtension) {
					...
					this.logger.info(
						localize('updateMessage', "Updating the extension '{0}' to the version {1}", id, gallery.version)
					);
				}
