...
export abstract class CommontExtensionManagementService extends Disposable implements IExtensionManagementService {
	...
	async canInstall(extension: IGalleryExtension): Promise<true | IMarkdownString> {
		...
		if (!(await this.isExtensionPlatformCompatible(extension))) {
			// Crow with Node.js host supports all platforms
	...
	implements IExtensionManagementService
{
	...
	private async checkAndGetCompatibleVersion(
		...
		if (deprecationInfo?.extension?.autoMigrate) {
			...
			if ((await this.canInstall(extension)) !== true) {
				if ((globalThis as any).__CROW_TAURI__ === true) {
					// Crow has a Node.js extension host — skip platform compatibility check
				} else {
	...
	protected async getCompatibleVersion(
		...
		if (!compatibleExtension && (globalThis as any).__CROW_TAURI__ === true) {
			compatibleExtension = extension;
		}
