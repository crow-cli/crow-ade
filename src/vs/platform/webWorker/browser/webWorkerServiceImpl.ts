...
export class WebWorkerService implements IWebWorkerService {
	...
	getWorkerUrl(descriptor: WebWorkerDescriptor): string {
		const isTauriProduction =
			(globalThis as typeof globalThis & { __CROW_TAURI__?: boolean }).__CROW_TAURI__ === true &&
	...
	const blob = new Blob(
		[
			coalesce([
				...
				`globalThis.__CROW_TAURI__ = ${JSON.stringify(!!(globalThis as any).__CROW_TAURI__)};`,
				`const ttPolicy = globalThis.trustedTypes?.createPolicy('defaultWorkerFactory', { createScriptURL: value => value });`,
				`globalThis.workerttPolicy = ttPolicy;`,
				`const __crowImportUrlRaw = (ttPolicy?.createScriptURL(${JSON.stringify(workerScriptUrl)}) ?? ${JSON.stringify(workerScriptUrl)});`,
				`let __crowImportUrl = String(__crowImportUrlRaw);`,
				`if (__crowImportUrl.startsWith('tauri://')) {`,
				`	const hashIndex = __crowImportUrl.indexOf('#');`,
				`	if (hashIndex >= 0) {`,
				`		__crowImportUrl = __crowImportUrl.slice(0, hashIndex);`,
				`	}`,
				`}`,
				workerLoadingFailedErrorMessage ? 'try {' : '',
				`await import(__crowImportUrl);`,
