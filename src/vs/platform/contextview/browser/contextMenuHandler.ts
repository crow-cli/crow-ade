		...
		this.contextViewService.showContextView(
			{
				...
				render: container => {
					...
					menuDisposables.add(
						addDisposableListener(targetWindow, EventType.BLUR, () => this.contextViewService.hideContextView(true))
					);
					menuDisposables.add(
						addDisposableListener(targetWindow, EventType.MOUSE_DOWN, (e: MouseEvent) => {
				...
				onHide: (didCancel?: boolean) => {
					...
					if (
						!!this.lastContainer &&
						(getActiveElement() === this.lastContainer || isAncestor(getActiveElement(), this.lastContainer))
					) {
	...
	private onActionRun(e: IRunEvent, logTelemetry: boolean): void {
		if (logTelemetry) {
			this.telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>(
				...
				{ id: e.action.id, from: 'contextMenu' }
			);
		}
