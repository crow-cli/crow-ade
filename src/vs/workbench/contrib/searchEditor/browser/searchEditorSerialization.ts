...
const matchToSearchResultFormat = (
	...
	fullMatchLines.forEach((sourceLine, i) => {
		...
		const rangeOnThisLine = ({ start, end }: { start?: number; end?: number }) =>
...
const contentPatternToSearchConfiguration = (
	...
	return {
		...
		useExcludeSettingsAndIgnoreFiles:
			pattern?.userDisabledExcludesAndIgnoreFiles === undefined ? true : !pattern.userDisabledExcludesAndIgnoreFiles,
		contextLines,
...
export const serializeSearchConfiguration = (config: Partial<SearchConfiguration>): string => {
	const removeNullFalseAndUndefined = <T>(a: (T | null | false | undefined)[]) =>
...
export const serializeSearchResultForEditor = (
	...
	limitHit?: boolean
): { matchRanges: Range[]; text: string; config: Partial<SearchConfiguration> } => {
	...
	const info = [searchResult.count() ? `${resultcount} - ${filecount}` : localize('noResults', 'No Results')];
	if (limitHit) {
		info.push(
			localize(
