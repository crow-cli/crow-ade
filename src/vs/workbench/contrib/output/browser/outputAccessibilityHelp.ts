...
class OutputAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	provideContent(): string {
		...
		lines.push(
			nls.localize('output.syntaxExclude', "- !text (exclude): Hides lines containing 'text', showing all other lines.")
		);
		lines.push(
			nls.localize('output.syntaxEscape', '- \\\\! (escape): Use backslash to search for a literal "!" character.')
		);
		lines.push(
			nls.localize(
