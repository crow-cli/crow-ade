...
class SearchAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	provideContent(): string {
		...
		content.push(
			localize('search.hear2', '2) The full line that contains the match is read, so you get immediate context.')
		);
		content.push(
			localize(
				'search.hear3',
		...
		content.push(
			localize('search.keyResultEnter', '- Enter (when a result is focused): Navigate to that result in the editor.')
		);
		content.push('');
		...
		content.push(
			localize('search.settingMaxResults', '- `search.maxResults`: Maximum number of search results to show.')
		);
		...
		if (isReplaceMode) {
			content.push(
				localize('search.settingReplacePreview', '- `search.useReplacePreview`: Open preview when replacing matches.')
			);
		}
