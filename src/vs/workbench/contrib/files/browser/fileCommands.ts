	...
	handler: async (accessor, resource: URI | object) => {
		...
		if (resources.length) {
			...
			const editors = files
				...
				.concat(
					...untitledResources.map(untitledResource => ({ resource: untitledResource, options: { pinned: true } }))
				);
	...
	handler: async (accessor, _resource: URI | object) => {
		...
		if (resources.length) {
			await editorService.openEditors(
				resources.map(r => ({ resource: r.resource, options: { preserveFocus: false, pinned: true } }))
			);
		}
	}
});
...
async function resourcesToClipboard(
	...
	if (resources.length) {
		...
		const copyRelativeOrFullPathSeparator: '/' | '\\' | undefined = configurationService.getValue(
			copyRelativeOrFullPathSeparatorSection
		);
			...
			if (editorsToRevert.length > 0) {
				actions.push(
					toAction({
						...
						label:
							editorsToRevert.length > 1 ? nls.localize('revertAll', 'Revert All') : nls.localize('revert', 'Revert'),
	...
	handler: async accessor => {
		...
		try {
			await editorService.revert(
				editors.filter(
					({ editor }) => !editor.hasCapability(EditorInputCapabilities.Untitled) /* all except untitled */
				),
				{ force: true }
			);
		} catch (error) {
...
CommandsRegistry.registerCommand({
	...
	handler: (accessor, resource: URI | object) => {
		...
		const resources = getMultiSelectedResources(
			...
			resource => workspace.folders.some(folder => uriIdentityService.extUri.isEqual(folder.uri, resource)) // Need to verify resources are workspaces since multi selection can trigger this command on some non workspace resources
		);
