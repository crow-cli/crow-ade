...
const onceFilter = (event: Event<ITaskEvent>, filter: (e: ITaskEvent) => boolean) =>
...
export class DebugTaskRunner implements IDisposable {
		...
		try {
			...
			if (checkboxChecked) {
				this.configurationService.updateValue(
					'debug.onTaskErrors',
					result === DebugChoice.DebugAnyway ? 'debugAnyway' : abort ? 'abort' : 'showErrors'
				);
			}
			...
			const choiceMap: { [key: string]: number } = JSON.parse(
				this.storageService.get(DEBUG_TASK_ERROR_CHOICE_KEY, StorageScope.WORKSPACE, '{}')
			);
	...
	async runTask(
		...
		const inactivePromise: Promise<ITaskSummary | null> = new Promise(resolve =>
			store.add(
		...
		store.add(
			onceFilter(
				...
				e =>
					(e.kind === TaskEventKind.Active || e.kind === TaskEventKind.DependsOnStarted) &&
					getTaskKey(e.__task) === taskKey
			)(() => {
			...
			store.add(
				didAcquireInput.event(() => {
					...
					const hideSlowPreLaunchWarning =
						this.configurationService.getValue<IDebugConfiguration>('debug').hideSlowPreLaunchWarning;
