...
export class ReplAccessibilityHelp implements IAccessibleViewImplementation {
	priority = 120;
	name = 'replHelp';
	when = ContextKeyExpr.or(ContextKeyExpr.equals('focusedView', 'workbench.panel.repl.view'), CONTEXT_IN_DEBUG_REPL);
...
class ReplAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	public provideContent(): string {
		...
		content.push(
			localize('repl.syntaxExclude', '- !text (exclude): Hides messages containing the text, showing all others.')
		);
		content.push('');
		...
		content.push(
			localize('repl.settingHistory', '- `debug.console.historySuggestions`: Suggest previously typed input.')
		);
		content.push(
			localize(
