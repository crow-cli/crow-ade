		...
		this._register(
			_configurationService.onDidChangeConfiguration(e => {
				if (e.affectsConfiguration('debug')) {
					...
					const newValue =
						this._configurationService.getValue<IDebugConfiguration>('debug').disassemblyView.showSourceCode;
				...
				[
					{
						...
						project(row: IDisassembledInstructionEntry): IDisassembledInstructionEntry {
							return row;
						}
					},
					{
		...
		this._register(
			this._disassembledInstructions.onDidScroll(e => {
				...
				if (e.oldScrollTop > e.scrollTop && e.scrollTop < e.height) {
					...
					this.scrollUp_LoadDisassembledInstructions(DisassemblyView.NUM_INSTRUCTIONS_TO_LOAD)
						.then(loaded => {
							if (loaded > 0) {
...
class InstructionRenderer
	extends Disposable
	implements ITableRenderer<IDisassembledInstructionEntry, IInstructionColumnTemplateData>
{
