...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor: ServicesAccessor, extId: string) => {
		...
		for (const test of result.tests) {
			if (
				TestId.compare(ownState.item.extId, test.item.extId) === TestPosition.IsChild &&
...
CommandsRegistry.registerCommand({
	...
	handler: async (
		...
		extId: string,
		opts?: { preserveFocus?: boolean; openToSide?: boolean }
	) => {
		...
		const position =
			accessor.get(ITestingDecorationsService).getDecoratedTestPosition(uri, extId) || range?.getStartPosition();
...
CommandsRegistry.registerCommand({
	...
	handler: async (accessor: ServicesAccessor, group: TestRunProfileBitset, ...testIds: string[]) => {
		...
		await discoverAndRunTests(accessor.get(ITestService).collection, accessor.get(IProgressService), testIds, tests =>
			testService.runTests({ group, tests })
		);
	}
});
