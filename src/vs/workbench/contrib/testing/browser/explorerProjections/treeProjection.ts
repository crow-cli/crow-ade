...
const computedStateAccessor: IComputedStateAndDurationAccessor<TreeTestItemElement> = {
	...
	getChildren: i =>
		Iterable.filter(i.children.values(), (t): t is TreeTestItemElement => t instanceof TreeTestItemElement),
	*getParents(i) {
...
export class TreeProjection extends Disposable implements ITestTreeProjection {
		...
		this._register(
			results.onTestChanged(ev => {
				...
				const refreshDuration =
					ev.reason === TestResultItemChangeReason.OwnStateChange && ev.previousOwnDuration !== result.ownDuration;
				...
				refreshComputedState(computedStateAccessor, item, explicitComputed, refreshDuration).forEach(i =>
					i.fireChange()
				);
			})
		);
			...
			switch (op.op) {
				...
				case TestDiffOpType.Remove: {
					...
					const affectsRootElement =
						toRemove.depth === 1 &&
						(parent?.children.size === 1 || !Iterable.some(this.rootsWithChildren, (_, i) => i === 1));
					...
					if (parent instanceof TreeTestItemElement) {
						refreshComputedState(computedStateAccessor, parent, undefined, !!parent.duration).forEach(i =>
							i.fireChange()
						);
					}
				}
	...
	public applyTo(tree: ObjectTree<TestExplorerTreeElement, FuzzyScore>) {
		for (const parent of this.changedParents) {
			if (!parent || tree.hasElement(parent)) {
				tree.setChildren(parent, getChildrenForParent(this.lastState, this.rootsWithChildren, parent), {
	...
	private storeItem(treeElement: TreeTestItemElement) {
		...
		if (prevState) {
			...
			refreshComputedState(computedStateAccessor, treeElement, undefined, !!treeElement.ownDuration).forEach(i =>
				i.fireChange()
			);
		}
	}
