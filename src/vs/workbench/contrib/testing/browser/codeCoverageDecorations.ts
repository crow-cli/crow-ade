...
export class CodeCoverageDecorations extends Disposable implements IEditorContribution {
	...
	constructor(
		...
		this._register(
			bindContextKey(TestingContextKeys.hasInlineCoverageDetails, contextKeyService, reader =>
				this.hasInlineCoverageDetails.read(reader)
			)
		);
	...
	private hoverInlineDecoration(model: ITextModel, position: Position) {
		...
		const decoration = mapFindFirst(allDecorations, ({ id }) =>
			this.decorationIds.has(id) ? { id, deco: this.decorationIds.get(id)! } : undefined
		);
...
type DetailRange = {
	range: Range;
	primary: boolean;
	metadata: { detail: CoverageDetailsWithBranch; description: IMarkdownString | undefined };
};
...
export class CoverageDetailsModel {
	...
	public describe(detail: CoverageDetailsWithBranch, model: ITextModel): IMarkdownString | undefined {
		if (detail.type === DetailType.Declaration) {
			...
			if (!count) {
				return new MarkdownString().appendMarkdown(
					localize('coverage.branchNotCovered', 'Branch {0} in {1} was not covered.', label2, text)
				);
			} else if (count === true) {
				return new MarkdownString().appendMarkdown(
					localize('coverage.branchCoveredYes', 'Branch {0} in {1} was executed.', label2, text)
				);
			} else {
				return new MarkdownString().appendMarkdown(
					localize('coverage.branchCovered', 'Branch {0} in {1} was executed {2} time(s).', label2, text, count)
				);
			}
		}
...
class CoverageToolbarWidget extends Disposable implements IOverlayWidget {
	...
	constructor(
		...
		this._register(
			dom.addStandardDisposableListener(this._domNode.root, dom.EventType.CONTEXT_MENU, e => {
	...
	private setActions() {
		...
		toggleAction.tooltip = this.keybindingService.appendKeybinding(
			TOGGLE_INLINE_COMMAND_TEXT,
		...
		this.actionBar.push(
			new ActionWithIcon('rerun', localize('testing.rerun', 'Rerun'), testingRerunIcon, !this.isRunning, () =>
				this.rerunTest()
			)
		);
	}
			...
			quickInputService
				.pick(items, {
					activeItem: items.find(
						(item): item is TItem => 'testId' in item && item.testId?.toString() === previousSelection?.toString()
					),
...
registerAction2(
	class ToggleCoverageInExplorer extends Action2 {
		constructor() {
			super({
				...
				menu: [{ id: MenuId.CommandPalette, when: TestingContextKeys.isTestCoverageOpen }]
			});
		}
...
registerAction2(
	class GoToNextMissedCoverageLine extends Action2 {
		constructor() {
			super({
...
registerAction2(
	class GoToPreviousMissedCoverageLine extends Action2 {
		constructor() {
			super({
