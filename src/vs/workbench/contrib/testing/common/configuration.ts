...
export const testingConfiguration: IConfigurationNode = {
	...
	properties: {
		[TestingConfigKeys.AutoOpenPeekView]: {
			...
			enum: [AutoOpenPeekViewWhen.FailureAnywhere, AutoOpenPeekViewWhen.FailureVisible, AutoOpenPeekViewWhen.Never],
		...
		[TestingConfigKeys.CountBadge]: {
			...
			enum: [TestingCountBadge.Failed, TestingCountBadge.Off, TestingCountBadge.Passed, TestingCountBadge.Skipped],
			enumDescriptions: [
...
export const getTestingConfiguration = <K extends TestingConfigKeys>(config: IConfigurationService, key: K) =>
	config.getValue<ITestingConfiguration[K]>(key);

export const observeTestingConfiguration = <K extends TestingConfigKeys>(config: IConfigurationService, key: K) =>
	observableFromEvent(config.onDidChangeConfiguration, () => getTestingConfiguration(config, key));
