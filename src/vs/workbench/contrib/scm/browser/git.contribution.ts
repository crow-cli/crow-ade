/*---------------------------------------------------------------------------------------------
 *  Tauri Git SCM Provider for Crow
...
class TauriGitResource implements ISCMResource {
	...
	async open(_preserveFocus: boolean): Promise<void> {
		const commandService = (globalThis as any).__crow_commandService;
		if (!commandService) {
			return;
		}
...
class TauriGitContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		(globalThis as any).__crow_quickInputService = quickInputService;
		this._init();
	}
	...
	private async _init(): Promise<void> {
		...
		(window as any).__crow_workspaceFolders = folders.map(f => f.uri.fsPath);
...
CommandsRegistry.registerCommand('git.init', async () => {
	try {
		...
		const folders = (window as any).__crow_workspaceFolders;
...
CommandsRegistry.registerCommand('git.openOnGitHub', async (_accessor, remoteUrl: string, hash: string) => {
	try {
		...
		const openerService = (globalThis as any).__crow_openerService;
		if (openerService) {
...
async function doGitClone(): Promise<void> {
	try {
		const quickInputService = (globalThis as any).__crow_quickInputService;
...
CommandsRegistry.registerCommand('git.commitAndPush', async () => {
	...
	try {
		const invoke = await getTauriInvoke();
		if (invoke) {
			const message = (window as any).__crow_scmInputValue?.() || '';
			if (!message.trim()) {
				return;
			}
