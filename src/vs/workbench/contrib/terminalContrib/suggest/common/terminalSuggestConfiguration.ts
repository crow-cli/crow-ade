 ...
 */
export function normalizeQuickSuggestionsConfig(
	config: ITerminalSuggestConfiguration['quickSuggestions']
): ITerminalQuickSuggestionsOptions {
...
export function registerTerminalSuggestProvidersConfiguration(providers?: Map<string, ITerminalSuggestProviderInfo>) {
	...
	for (const id of Array.from(providers.keys()).sort()) {
		providersProperties[id] = {
			...
			description:
				providers.get(id)?.description ?? localize('suggest.provider.title', 'Show suggestions from {0}.', id)
		};
	}
