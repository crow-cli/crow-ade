...
class TerminalFindAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	provideContent(): string {
		...
		content.push(
			localize('terminal.focusDesc2', 'The terminal automatically scrolls to show the match you navigate to.')
		);
		content.push(
			localize(
				'terminal.focusDesc3',
		...
		content.push(
			localize('terminal.keyShiftEnter', '- Shift+Enter: Move to the previous match while staying in the Find input.')
		);
		content.push('');
		content.push(
			localize(
