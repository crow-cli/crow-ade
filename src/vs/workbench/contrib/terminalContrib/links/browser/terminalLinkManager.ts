...
export class TerminalLinkManager extends DisposableStore {
	...
	constructor(
		...
		const enableFileLinksConfig = this._configurationService.getValue<ITerminalConfiguration>(TERMINAL_CONFIG_SECTION)
		...
		this._setupLinkDetector(
			...
			this.add(this._instantiationService.createInstance(TerminalWordLinkDetector, this._xterm))
		);
		...
		const localFolderOutsideWorkspaceOpener = this._instantiationService.createInstance(
			TerminalLocalFolderOutsideWorkspaceLinkOpener
		);
		...
		this._openers.set(
			...
			this._instantiationService.createInstance(
				...
				localFileOpener,
				localFolderInWorkspaceOpener,
				localFolderOutsideWorkspaceOpener
			)
		);
							...
							[
								{
									...
									run: () => {
										const allowedLinkSchemes = [...terminalConfigurationService.config.allowedLinkSchemes, scheme];
	...
	private _setupLinkDetector(id: string, detector: ITerminalLinkDetector, isExternal: boolean = false): ILinkProvider {
		...
		this.add(
			detectorAdapter.onDidShowHover(e =>
				this._tooltipCallback(e.link, e.viewportRange, e.modifierDownCallback, e.modifierUpCallback)
			)
		);
		if (!isExternal) {
	...
	async getLinks(): Promise<{ viewport: IDetectedLinks; all: Promise<IDetectedLinks> }> {
		...
		const allLinks: Promise<Required<Pick<IDetectedLinks, 'wordLinks' | 'webLinks' | 'fileLinks' | 'folderLinks'>>> =
			Promise.all(aboveViewportLinksPromises).then(async aboveViewportLinks => {
		...
		switch (type) {
			case 'word':
				return await new Promise<ILink[] | undefined>(r =>
					this._standardLinkProviders.get(TerminalWordLinkDetector.id)?.provideLinks(y, r)
				);
			case 'url':
				return await new Promise<ILink[] | undefined>(r =>
					this._standardLinkProviders.get(TerminalUriLinkDetector.id)?.provideLinks(y, r)
				);
			case 'localFile': {
				const links = await new Promise<ILink[] | undefined>(r =>
					this._standardLinkProviders.get(TerminalLocalLinkDetector.id)?.provideLinks(y, r)
				);
	...
	private _registerStandardLinkProviders(): void {
		...
		const wrappedLinkProvider = this._setupLinkDetector(
			detectorId,
			new TerminalExternalLinkDetector(detectorId, this._xterm, proxyLinkProvider),
			true
		);
