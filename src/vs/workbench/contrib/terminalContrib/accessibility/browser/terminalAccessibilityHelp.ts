...
export class TerminalAccessibilityHelpProvider extends Disposable implements IAccessibleViewContentProvider {
	...
	onClose() {
		const expr = ContextKeyExpr.and(
			accessibleViewIsShown,
			ContextKeyExpr.equals(accessibleViewCurrentProviderId.key, AccessibleViewProviderId.TerminalHelp)
		);
	...
	constructor(
		private readonly _instance: Pick<
			ITerminalInstance,
			'shellType' | 'capabilities' | 'onDidRequestFocus' | 'resource' | 'focus'
		>,
		...
		if (this._configurationService.getValue(TerminalSuggestSettingId.Enabled)) {
			...
			content.push(
				localize(
					...
					TerminalSuggestCommandId.LearnMore
				)
			);
			content.push(
				localize(
