	...
	async provideCompletionItems(
		...
		for (const snippet of snippets) {
			...
			for (const anchor of anchors) {
				...
				if (
					isPatternInWord(anchor.prefixLow, 0, anchor.prefixLow.length, snippet.prefixLow, 0, snippet.prefixLow.length)
				) {
					candidate = anchor;
					break;
				}
		...
		if (
			!triggerCharacterLow &&
			(/\s/.test(lineContentLow[position.column - 2]) /*end in whitespace */ || !lineContentLow) /*empty line*/
		) {
			for (const snippet of snippets) {
