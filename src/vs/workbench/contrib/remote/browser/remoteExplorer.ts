...
export class RemoteExplorerViewPane extends ViewPane {
	...
	private buildTunnelSection(active: { label: string; kind: string }[], githubToken: string | null): RemoteSectionItem {
		...
		children.push({
			...
			onActivate: () => this.commandService.executeCommand('crow.remote.signInTunnel', 'microsoft')
		});
		...
		if (!githubToken) {
			children.push({
				...
				description: localize('remote.tunnelsProvider', 'Remote-Tunnels'),
				actionLabel: localize('remote.signIn', 'Sign In'),
				onActivate: () => this.commandService.executeCommand('crow.remote.signInTunnel', 'github')
			});
		}
		...
		if (hosts.length === 0) {
			...
			for (const host of hosts) {
				...
				children.push({
					...
					icon: Codicon.vm,
					active: isConnected,
					onActivate: () => this.commandService.executeCommand('crow.remote.connect', 'ssh' as RemoteKind, host)
				});
			}
		}
				...
				children.push({
					...
					onActivate: async () => {
						try {
							...
							this.commandService.executeCommand('crow.notify.error', String(err));
						}
					}
				});
			}
	...
	private buildWslSection(distros: WslDistro[]): RemoteSectionItem {
		const children: RemoteLeafItem[] = distros.map(distro => ({
			...
			active: false,
			onActivate: () => this.commandService.executeCommand('crow.remote.connect', 'wsl' as RemoteKind, distro)
		}));
		...
		if (ctrs.length === 0) {
			...
			for (const ctr of ctrs) {
				...
				children.push({
					...
					onActivate: () =>
						isRunning
							? this.commandService.executeCommand('crow.remote.connect', 'container' as RemoteKind, ctr)
							: undefined
				});
			}
		}
