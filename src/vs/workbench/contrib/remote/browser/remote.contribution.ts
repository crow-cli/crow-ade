/*---------------------------------------------------------------------------------------------
 *  Crow — Remote workbench contribution.`
 *--------------------------------------------------------------------------------------------*/
...
import {
	...
	ContainerEntry,
	RemoteKind
} from '../../../../platform/crow/browser/crowRemoteService.js';
...
registerAction2(
	class RefreshRemoteExplorer extends Action2 {
		constructor() {
			super({
				id: 'crow.remote.refresh',
				title: localize2('crow.remote.refresh', 'Refresh Remote Explorer'),
...
registerAction2(
	class ConnectRemote extends Action2 {
		constructor() {
			super({ id: 'crow.remote.connect', title: localize2('crow.remote.connect', 'Connect to Remote') });
		}
		async run(
			...
			const remoteService = accessor.get(ICrowRemoteService);
...
registerAction2(
	class OpenRemoteExplorer extends Action2 {
		constructor() {
			super({
				id: 'crow.remote.openExplorer',
				title: localize2('crow.remote.openExplorer', 'Remote Explorer'),
...
registerAction2(
	class OpenRemoteWindow extends Action2 {
		constructor() {
			super({
				id: 'crow.remote.openWindow',
				title: localize2('crow.remote.openWindow', 'Connect to…'),
				menu: [{ id: MenuId.CommandPalette }]
			});
		}
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const picks: RemoteOption[] = [
				{
					...
					action: () => commandService.executeCommand('crow.remote.signInTunnel', 'github')
				},
				{
...
registerAction2(
	class SignInTunnel extends Action2 {
		constructor() {
			super({
				id: 'crow.remote.signInTunnel',
				title: localize2('crow.remote.signInTunnel', 'Sign in to Remote Tunnels')
			});
		}
...
async function pickAndConnectSsh(
	...
	remoteService: ICrowRemoteService
): Promise<void> {
...
async function storeGitHubToken(token: string): Promise<void> {
	...
	await invoke('secret_set', {
		key: 'crow.remote.github.device-flow',
		value: token
	});
}
...
class RemoteStatusBarIndicator extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		@ICrowRemoteService private readonly remoteService: ICrowRemoteService
	) {
		super();
	...
	private buildEntry(label: string | null): IStatusbarEntry {
		...
		return {
			...
			command: 'crow.remote.openWindow'
		};
	}
