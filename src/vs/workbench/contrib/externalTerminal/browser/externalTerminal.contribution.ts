				...
				if (useIntegratedTerminal) {
					...
					for (const cwd of cwds) {
						...
						if (
							...
							(resources.length === 1 || !resource || cwd.path === resource.path || cwd.path === dirname(resource.path))
						) {
					...
					distinct(
						targets.map(({ stat }) => (stat!.isDirectory ? stat!.resource.fsPath : dirname(stat!.resource.fsPath)))
					).forEach(cwd => {
...
export class ExternalTerminalContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(@IConfigurationService private readonly _configurationService: IConfigurationService) {
		...
		const shouldShowIntegratedOnLocal = ContextKeyExpr.and(
			...
			ContextKeyExpr.or(
				ContextKeyExpr.equals('config.terminal.explorerKind', 'integrated'),
		...
		const shouldShowExternalKindOnLocal = ContextKeyExpr.and(
			ResourceContextKey.Scheme.isEqualTo(Schemas.file),
			ContextKeyExpr.or(
				ContextKeyExpr.equals('config.terminal.explorerKind', 'external'),
				ContextKeyExpr.equals('config.terminal.explorerKind', 'both')
			)
		);
