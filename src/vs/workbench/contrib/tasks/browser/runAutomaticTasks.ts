...
export class RunAutomaticTasks extends Disposable implements IWorkbenchContribution {
	...
	): { tasks: Array<Task | Promise<Task | undefined>>; taskNames: Array<string>; locations: Map<string, URI> } {
	...
	private async _runWithPermission(
		...
		const hasShownPromptForAutomaticTasks = storageService.getBoolean(
		...
		const allow = await this._showPrompt(
			notificationService,
			storageService,
			openerService,
		...
		return new Promise<boolean>(resolve => {
			notificationService.prompt(
				...
				[
					...
					{
						label:
							locations.size === 1 ? nls.localize('openTask', 'Open File') : nls.localize('openTasks', 'Open Files'),
						run: async () => {
