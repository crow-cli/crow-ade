	...
	implements MainThreadTunnelServiceShape, PortAttributesProvider
{
	...
	private processFindingEnabled(): boolean {
		return (
			(!!this.configurationService.getValue(PORT_AUTO_FORWARD_SETTING) || this.tunnelService.hasTunnelProvider) &&
		...
		this._register(
			this.configurationService.onDidChangeConfiguration(async e => {
				if (
					...
					(e.affectsConfiguration(PORT_AUTO_FORWARD_SETTING) || e.affectsConfiguration(PORT_AUTO_SOURCE_SETTING))
				) {
	...
	async providePortAttributes(
		...
		const appropriateHandles = Array.from(this.portsAttributesProviders.entries())
			.filter(entry => {
				...
				const portRange =
					typeof selector.portRange === 'number' ? [selector.portRange, selector.portRange + 1] : selector.portRange;
			...
			[
				{
					...
					run: async () => {
						...
						await this.remoteExplorerService.close(
							{ host: tunnel.tunnelRemoteHost, port: tunnel.tunnelRemotePort },
					...
					return {
						...
						dispose: async (silent?: boolean) => {
							...
							return this._proxy.$closeTunnel(
								{ host: tunnel.remoteAddress.host, port: tunnel.remoteAddress.port },
								silent
							);
						}
					};
