...
CommandsRegistry.registerCommand(
	...
	function (accessor: ServicesAccessor, uri: UriComponents | string): Promise<boolean> {
...
CommandsRegistry.registerCommand(
	...
	function (accessor: ServicesAccessor, toOpen: IWindowOpenable[], options: IOpenWindowOptions) {
...
CommandsRegistry.registerCommand('_remoteCLI.getSystemStatus', function (accessor: ServicesAccessor): Promise<
	string | undefined
> {
	const commandService = accessor.get(ICommandService);
...
CommandsRegistry.registerCommand(
	...
	async function (accessor: ServicesAccessor, args: ManageExtensionsArgs): Promise<string | undefined> {
		...
		const childInstantiationService = instantiationService.createChild(
			new ServiceCollection([IExtensionManagementService, remoteExtensionManagementService])
		);
		try {
...
class RemoteExtensionManagementCLI extends ExtensionManagementCLI {
	...
	protected override validateExtensionKind(manifest: IExtensionManifest): boolean {
		if (
			!this._extensionManifestPropertiesService.canExecuteOnWorkspace(manifest) &&
