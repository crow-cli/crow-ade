					...
					},
					{
						...
						properties: {
							...
							tags: {
								...
								items: {
									type: 'string',
									enum: ['accessibility', 'advanced', 'experimental', 'telemetry', 'usesOnlineServices'],
									enumDescriptions: [
...
const defaultConfigurationExtPoint = ExtensionsRegistry.registerExtensionPoint<
	IStringDictionary<IStringDictionary<unknown>>
>({
...
defaultConfigurationExtPoint.setHandler((extensions, { added, removed }) => {
	...
	if (removed.length) {
		const removedDefaultConfigurations = removed.map<IConfigurationDefaults>(extension => ({
			...
			source: { id: extension.description.identifier.value, displayName: extension.description.displayName }
		}));
...
const extensionConfigurations: ExtensionIdentifierMap<IConfigurationNode[]> = new ExtensionIdentifierMap<
	IConfigurationNode[]
>();
	...
	function handleConfiguration(node: IConfigurationNode, extension: IExtensionPointUser<unknown>): IConfigurationNode {
		...
		configuration.restrictedProperties =
			extension.description.capabilities?.untrustedWorkspaces?.supported === 'limited'
				...
				: undefined;
		configuration.title =
			configuration.title || extension.description.displayName || extension.description.identifier.value;
		...
		if (properties) {
			...
			for (const key in properties) {
				...
				if (
					...
					!EXTENSION_UNIFICATION_EXTENSION_IDS.has(extension.description.identifier.value.toLowerCase())
				) {
					delete properties[key];
					extension.collector.warn(
						nls.localize(
				...
				if (!isObject(propertyConfiguration)) {
					...
					extension.collector.error(
						nls.localize('invalid.property', "configuration.properties property '{0}' must be an object", key)
					);
					continue;
				}
...
class ConfigurationDefaultsTableRenderer extends Disposable implements IExtensionFeatureTableRenderer {
	...
	render(manifest: IExtensionManifest): IRenderedData<ITableData> {
		...
		const headers = [
			nls.localize('language', 'Languages'),
			nls.localize('setting', 'Setting'),
			nls.localize('default override value', 'Override Value')
		];
		const rows: IRowData[][] = [];
