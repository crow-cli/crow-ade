/*---------------------------------------------------------------------------------------------
 *  Crow — ExtHost search stub. Search is routed through TauriSearchService; extension-
...
export class ExtHostSearch implements IExtHostSearch {
	...
	private _registerNoop(set: Set<string>, scheme: string, kind: string): IDisposable {
		...
		this._logService.trace(
			`[Crow] extHostSearch: ignoring ${kind} provider for scheme '${scheme}' (search routed through Rust)`
		);
