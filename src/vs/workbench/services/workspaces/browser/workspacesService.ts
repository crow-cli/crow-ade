		...
		switch (this.contextService.getWorkbenchState()) {
			...
			case WorkbenchState.WORKSPACE:
				this.addRecentlyOpened([
					{ workspace: { id: workspace.id, configPath: workspace.configuration! }, remoteAuthority }
				]);
				break;
		}
	}
	...
	async getRecentlyOpened(): Promise<IRecentlyOpened> {
		...
		if (recentlyOpenedRaw) {
			...
			recentlyOpened.workspaces = recentlyOpened.workspaces.filter(recent => {
				...
				if (
					!(globalThis as any).__CROW_TAURI__ &&
	...
	private doRemoveRecentlyOpened(recentlyOpened: IRecentlyOpened, paths: URI[]): void {
		...
		recentlyOpened.workspaces = recentlyOpened.workspaces.filter(workspace => {
			return !paths.some(
				path =>
					path.toString() ===
					(isRecentFolder(workspace) ? workspace.folderUri.toString() : workspace.workspace.configPath.toString())
			);
		});
	}
