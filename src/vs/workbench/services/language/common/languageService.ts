			...
			for (let i = 0, len = extensions.length; i < len; i++) {
				...
				if (!Array.isArray(extension.value)) {
					extension.collector.error(
						localize('invalid', 'Invalid `contributes.{0}`. Expected an array.', languagesExtPoint.name)
					);
					continue;
				}
...
function isValidLanguageExtensionPoint(
	...
	if (!isUndefinedOrStringArray(value.extensions)) {
		collector?.error(
			localize('opt.extensions', 'property `{0}` can be omitted and must be of type `string[]`', 'extensions')
		);
		return false;
	}
	if (!isUndefinedOrStringArray(value.filenames)) {
		collector?.error(
			localize('opt.filenames', 'property `{0}` can be omitted and must be of type `string[]`', 'filenames')
		);
		return false;
	}
	if (typeof value.firstLine !== 'undefined' && typeof value.firstLine !== 'string') {
		collector?.error(
			localize('opt.firstLine', 'property `{0}` can be omitted and must be of type `string`', 'firstLine')
		);
		return false;
	}
	if (typeof value.configuration !== 'undefined' && typeof value.configuration !== 'string') {
		collector?.error(
			localize('opt.configuration', 'property `{0}` can be omitted and must be of type `string`', 'configuration')
		);
		return false;
	}
	if (!isUndefinedOrStringArray(value.aliases)) {
		collector?.error(
			localize('opt.aliases', 'property `{0}` can be omitted and must be of type `string[]`', 'aliases')
		);
		return false;
	}
	if (!isUndefinedOrStringArray(value.mimetypes)) {
		collector?.error(
			localize('opt.mimetypes', 'property `{0}` can be omitted and must be of type `string[]`', 'mimetypes')
		);
		return false;
	}
