	...
	implements IWorkbenchExtensionManagementService
{
	...
	async installResourceExtension(
		...
		if (!((globalThis as any).__CROW_TAURI__ === true) && !this.canInstallResourceExtension(extension)) {
	...
	private async checkInstallingExtensionOnWeb(
		...
		// Crow: when running inside Tauri with a Node.js extension host available,
