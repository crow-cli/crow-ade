...
export class WebExtensionsScannerService extends Disposable implements IWebExtensionsScannerService {
	...
	private async toWebExtension(
		...
		const isCrowTauri = (globalThis as any).__CROW_TAURI__ === true;
		if (!isCrowTauri && !this.extensionManifestPropertiesService.canExecuteOnWeb(manifest)) {
			throw new Error(
				localize(
					'not a web extension',
	...
	private async toScannedExtension(
		...
		return {
			...
			targetPlatform: (globalThis as any).__CROW_TAURI__ === true ? TargetPlatform.UNDEFINED : TargetPlatform.WEB,
			validations,
			isValid,
