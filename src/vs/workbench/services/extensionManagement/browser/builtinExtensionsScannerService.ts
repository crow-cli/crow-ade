	...
	constructor(
		...
		if (isWeb) {
			...
			this.logService.info(
				`[Crow-Builtin] builtinExtensionsServiceUrl = ${builtinExtensionsServiceUrl?.toString?.()}`
			);
			if (builtinExtensionsServiceUrl) {
				...
				if ((globalThis as any)._VSCODE_BUILTIN_EXTENSIONS) {
					...
					this.logService.info(
						`[Crow-Builtin] loaded ${bundledExtensions.length} extensions from _VSCODE_BUILTIN_EXTENSIONS`
					);
				} else {
					...
					const builtinExtensionsElementAttribute = builtinExtensionsElement
						...
						: undefined;
					this.logService.info(
						`[Crow-Builtin] DOM meta element present? ${!!builtinExtensionsElement} data-settings length? ${builtinExtensionsElementAttribute?.length ?? 0}`
					);
					if (builtinExtensionsElementAttribute) {
						try {
							bundledExtensions = JSON.parse(builtinExtensionsElementAttribute);
							this.logService.info(`[Crow-Builtin] parsed ${bundledExtensions.length} extensions from DOM`);
						} catch (_error) {
							/* ignore error*/
						}
					}
