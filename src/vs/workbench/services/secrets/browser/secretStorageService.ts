/*---------------------------------------------------------------------------------------------
 *  Crow - Secret storage backed by the OS keyring.
 *
 *  All secret reads and writes flow through the `crow-auth` crate, which
...
export class BrowserSecretStorageService extends BaseSecretStorageService {
	...
	private async invoke<T>(cmd: string, args?: Record<string, unknown>): Promise<T | undefined> {
		...
		try {
			...
			this._logService.warn(`[crow-auth] ${cmd} failed`, error);
			return undefined;
		}
	}
