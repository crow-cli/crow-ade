/*---------------------------------------------------------------------------------------------
 *  Crow — Native Rust textmate tokenization, wired as ITextMateTokenizationService.
 *--------------------------------------------------------------------------------------------*/
...
import { getNativeTextMate } from '../../../../platform/crow/browser/crowTextMateService.js';
...
class CrowNativeBackgroundTokenizer implements IBackgroundTokenizer {
...
class CrowNativeTokenizationSupport implements ITokenizationSupport, IDisposable {
	constructor(
		private readonly _scopeName: string,
		private readonly _encodedLanguageId: LanguageId,
		private readonly _maxTokenizationLineLength: IObservable<number>
	...
	tokenize(_line: string, _hasEOL: boolean, _state: IState): TokenizationResult {
		throw new Error('CrowNativeTokenizationSupport: plain tokenize() not supported');
	}
	...
	createBackgroundTokenizer(
		...
		return new CrowNativeBackgroundTokenizer(
			textModel,
			store,
			this._scopeName,
	...
	dispose(): void {
		// Nothing to release — all per-model state lives in CrowNativeBackgroundTokenizer.
	}
}
...
export class CrowTextMateTokenizationFeature extends Disposable implements ITextMateTokenizationService {
	...
	public startDebugMode(_printFn: (str: string) => void, _onStop: () => void): void {
		this._notificationService.info(
			nls.localize('crow.textmate.noDebug', 'TextMate debug mode is not available with the native Rust tokenizer.')
		);
	}
	...
	private async _createNativeTokenizationSupport(
		...
		try {
			...
			this._logService.error(`[Crow-TextMate] Failed to load grammar for ${languageId} (${scopeName}):`, err);
			return null;
		}
		...
		return new CrowNativeTokenizationSupport(scopeName, encodedLanguageId, maxTokenizationLineLength);
	}
	...
	private _updateTheme(colorTheme: IWorkbenchColorTheme, forceUpdate: boolean): void {
		...
		getNativeTextMate()
			...
			.catch(err => {
				this._logService.error('[Crow-TextMate] Failed to update theme:', err);
			});
	}
