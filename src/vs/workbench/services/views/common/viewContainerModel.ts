...
export class ViewContainerModel extends Disposable implements IViewContainerModel {
	...
	private _onDidChangeAllViewDescriptors = this._register(
		new Emitter<{ added: ReadonlyArray<IViewDescriptor>; removed: ReadonlyArray<IViewDescriptor> }>()
	);
	...
	private updateContainerInfo(): void {
		...
		const useDefaultContainerInfo =
			...
			this.visibleViewDescriptors.some(
				v => Registry.as<IViewsRegistry>(ViewExtensions.ViewsRegistry).getViewContainer(v.id) === this.viewContainer
			);
		const title = useDefaultContainerInfo
			...
			: this.visibleViewDescriptors[0]?.containerTitle || this.visibleViewDescriptors[0]?.name?.value || '';
	...
	private findAndIgnoreIfNotFound(
		id: string
	): { index: number; visibleIndex: number; viewDescriptorItem: IViewDescriptorItem } | undefined {
