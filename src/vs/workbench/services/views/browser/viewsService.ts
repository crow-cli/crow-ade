...
export class ViewsService extends Disposable implements IViewsService {
	...
	private readonly _onDidChangeViewVisibility: Emitter<{ id: string; visible: boolean }> = this._register(
	...
	private readonly _onDidChangeViewContainerVisibility = this._register(
		new Emitter<{ id: string; visible: boolean; location: ViewContainerLocation }>()
	);
	...
	private onDidRegisterViewContainer(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): void {
		...
		disposables.add(
			viewContainerModel.onDidChangeAllViewDescriptors(({ added, removed }) => {
	...
	private onDidChangeContainerLocation(
		...
		if (
			...
			this.viewDescriptorService.getViewContainersByLocation(to).filter(vc => this.isViewContainerActive(vc.id))
				.length === 1
		) {
	...
	private updateViewContainerEnablementContextKey(viewContainer: ViewContainer): void {
		...
		contextKey.set(
			!(
				...
				this.viewDescriptorService.getViewContainerModel(viewContainer).activeViewDescriptors.length === 0
			)
		);
	}
	...
	async closeViewContainer(id: string): Promise<void> {
		...
		if (viewContainer) {
			const viewContainerLocation = this.viewDescriptorService.getViewContainerLocation(viewContainer);
			const isActive =
				viewContainerLocation !== null && this.paneCompositeService.getActivePaneComposite(viewContainerLocation);
			if (viewContainerLocation !== null) {
				return isActive
					? this.layoutService.setPartHidden(true, this.paneCompositeService.getPartId(viewContainerLocation))
					: undefined;
			}
		}
	}
						...
						public async run(serviceAccessor: ServicesAccessor): Promise<void> {
							...
							switch (viewContainerLocation) {
								...
								case ViewContainerLocation.Panel:
									if (
										!viewsService.isViewContainerVisible(viewContainer.id) ||
					...
					MenuRegistry.appendMenuItem(MenuId.MenubarViewMenu, {
						...
						group:
							defaultLocation === ViewContainerLocation.Sidebar
								? '3_sidebar'
								: defaultLocation === ViewContainerLocation.AuxiliaryBar
									? '4_auxbar'
									: '5_panel',
	...
	private registerPaneComposite(viewContainer: ViewContainer, viewContainerLocation: ViewContainerLocation): void {
		...
		Registry.as<PaneCompositeRegistry>(
			this.paneCompositeService.getRegistryId(viewContainerLocation)
	...
	private createViewPaneContainer(
		...
		const viewPaneContainer: ViewPaneContainer = instantiationService.createInstance(
