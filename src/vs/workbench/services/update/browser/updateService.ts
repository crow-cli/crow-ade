...
const STATE_EVENT = 'crow://update/state-change';
...
export class CrowUpdateService implements IUpdateService {
	...
	private async _bootstrap(): Promise<void> {
		...
		try {
			...
			console.error('[crow-update] bootstrap failed:', err);
			this.setState(State.Idle(UpdateType.Archive));
		}
	}
	...
	async checkForUpdates(explicit: boolean): Promise<void> {
		...
		try {
			...
			console.error('[crow-update] check failed:', err);
		}
	}
	...
	async downloadUpdate(explicit: boolean): Promise<void> {
		...
		try {
			...
			console.error('[crow-update] download failed:', err);
		}
	}
	...
	async applyUpdate(): Promise<void> {
		...
		try {
			...
			console.error('[crow-update] apply failed:', err);
		}
	}
	...
	async quitAndInstall(): Promise<void> {
		...
		try {
			...
			console.error('[crow-update] quit-and-install failed:', err);
		}
	}

	async _applySpecificUpdate(_packagePath: string): Promise<void> {
		// Crow doesn't expose side-loaded update packages; the VS Code flow
...
registerSingleton(IUpdateService, CrowUpdateService, InstantiationType.Eager);
