...
function createBuiltInColorThemes(): ColorThemeData[] {
	return [
		ColorThemeData.createLoadedTheme(
			`${ThemeTypeSelector.VS_DARK} crow-builtin-dark-modern`,
			'Dark Modern',
		...
		ColorThemeData.createLoadedTheme(
			`${ThemeTypeSelector.VS} crow-builtin-light-modern`,
			'Light Modern',
		...
		ColorThemeData.createLoadedTheme(
			`${ThemeTypeSelector.HC_BLACK} crow-builtin-hc-black`,
		...
		ColorThemeData.createLoadedTheme(
			`${ThemeTypeSelector.HC_LIGHT} crow-builtin-hc-light`,
...
export class WorkbenchThemeService extends Disposable implements IWorkbenchThemeService {
	...
	private static readonly AUTO_DETECT_MIGRATION_KEY = 'crow.migration.v1.disableAutoDetectColorScheme';
