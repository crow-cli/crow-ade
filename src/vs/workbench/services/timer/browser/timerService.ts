...
export abstract class AbstractTimerService implements ITimerService {
	...
	constructor(
		...
		Promise.all([
			...
			Promise.all(
				Array.from(Registry.as<ITerminalBackendRegistry>(TerminalExtensions.Backend).backends.values()).map(
					e => e.whenReady
				)
			)
		])
	...
	private async _computeStartupMetrics(): Promise<IStartupMetrics> {
		...
		const activeAuxiliaryViewlet = this._paneCompositeService.getActivePaneComposite(
		...
		const info: Writeable<IStartupMetrics> = {
			...
			timers: {
				...
				ellapsedAuxiliaryViewletRestore: this._marks.getDuration(
					'code/willRestoreAuxiliaryBar',
					'code/didRestoreAuxiliaryBar'
				),
