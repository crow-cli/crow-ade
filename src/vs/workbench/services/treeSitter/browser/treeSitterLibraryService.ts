/*---------------------------------------------------------------------------------------------
 *  Crow - Tree-sitter library service stub.
 *
 *  Native tree-sitter parsing lives in the `crow-syntax` Rust crate and is
...
export class TreeSitterLibraryService extends Disposable implements ITreeSitterLibraryService {
	...
	async getParserClass(): Promise<typeof Parser> {
		throw new Error('[crow] tree-sitter WASM parser disabled; use the Rust backend');
	}
