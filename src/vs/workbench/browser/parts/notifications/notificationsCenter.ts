...
export class NotificationsCenter extends Themable implements INotificationsCenterController {
	...
	show(): void {
		...
		// Make visible
		const [notificationsList, notificationsCenterContainer] = assertReturnsAllDefined(
	...
	private create(): void {
							...
							this.instantiationService.createInstance(
								...
								action,
								{
									getActions: () =>
										Separator.join(
											...this.menuService
												.getMenuActions(MenuId.NotificationsCenterPositionMenu, this.contextKeyService)
												.map(([, actions]) => actions)
										)
								},
									...
									getActions() {
										const actions = [
											toAction({
												...
												label:
													that.notificationService.getFilter() === NotificationsFilter.OFF
		...
		const configureNotificationsPositionAction = this._register(
			this.instantiationService.createInstance(
				ConfigureNotificationsPositionAction,
		...
		this.hideAction = this._register(
			this.instantiationService.createInstance(
				HideNotificationsCenterAction,
