...
export class NotificationTemplateRenderer extends Disposable {
	...
	private static updateExpandCollapseIcons(configurationService: IConfigurationService): void {
		...
		NotificationTemplateRenderer.expandNotificationAction.class = ThemeIcon.asClassName(
			getNotificationExpandIcon(position)
		);
		NotificationTemplateRenderer.collapseNotificationAction.class = ThemeIcon.asClassName(
			getNotificationCollapseIcon(position)
		);
	}
	...
	constructor(
		...
		if (!NotificationTemplateRenderer.closeNotificationAction) {
			NotificationTemplateRenderer.closeNotificationAction = instantiationService.createInstance(
				ClearNotificationAction,
				ClearNotificationAction.ID,
				ClearNotificationAction.LABEL
			);
			NotificationTemplateRenderer.expandNotificationAction = instantiationService.createInstance(
				ExpandNotificationAction,
				ExpandNotificationAction.ID,
				ExpandNotificationAction.LABEL
			);
			NotificationTemplateRenderer.collapseNotificationAction = instantiationService.createInstance(
				CollapseNotificationAction,
	...
	private render(notification: INotificationViewItem): void {
		...
		this.inputDisposables.add(
			addDisposableListener(this.template.container, EventType.MOUSE_UP, e => {
		...
		this.inputDisposables.add(
			addDisposableListener(this.template.container, EventType.AUXCLICK, e => {
	...
	private renderSecondaryActions(notification: INotificationViewItem, messageOverflows: boolean): void {
		...
		actions.forEach(action =>
			this.template.toolbar.push(action, { icon: true, label: false, keybinding: this.getKeybindingLabel(action) })
		);
	}
	...
	private renderButtons(notification: INotificationViewItem): void {
		...
		if (notification.expanded && isNonEmptyArray(primaryActions)) {
			...
			const actionRunner: IActionRunner = this.inputDisposables.add(
