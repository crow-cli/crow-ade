...
export class PaneCompositeBar extends Disposable {
	...
	private onDidChangeViewContainers(
		added: readonly { container: ViewContainer; location: ViewContainerLocation }[],
		removed: readonly { container: ViewContainer; location: ViewContainerLocation }[]
	) {
		removed
			...
			if (viewContainer) {
				...
				compositeActions = {
					...
					pinnedAction: this._register(
						new ToggleCompositePinnedAction(this.toCompositeBarActionItemFrom(viewContainerModel), this.compositeBar)
					),
					badgeAction: this._register(
						new ToggleCompositeBadgeAction(this.toCompositeBarActionItemFrom(viewContainerModel), this.compositeBar)
					)
				};
			} else {
	...
	private shouldBeHidden(
		...
		if (
			...
			!(this.part === Parts.SIDEBAR_PART && this.environmentService.remoteAuthority && isNative)
		) {
			...
			if (cachedViewContainer?.views?.length) {
				return cachedViewContainer.views.every(
					({ when }) => !!when && !this.contextKeyService.contextMatchesRules(ContextKeyExpr.deserialize(when))
				);
			}
		}
	...
	private getViewContainer(id: string): ViewContainer | undefined {
		...
		return viewContainer && this.viewDescriptorService.getViewContainerLocation(viewContainer) === this.location
			? viewContainer
			: undefined;
	}
		...
		for (const compositeItem of compositeItems) {
			...
			if (viewContainer) {
				...
				state.push({
					...
					icon:
						URI.isUri(viewContainerModel.icon) && this.environmentService.remoteAuthority
							? undefined
...
class ViewContainerActivityAction extends CompositeBarAction {
	...
	override async run(event: { preserveFocus: boolean }): Promise<void> {
		...
		const now = Date.now();
		if (
			now > this.lastRun /* https://github.com/microsoft/vscode/issues/25830 */ &&
