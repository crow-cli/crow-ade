		...
		const titleToolbar = instantiationService.createInstance(MenuWorkbenchToolBar, this.element, MenuId.CommandCenter, {
			...
			actionViewItemProvider: (action, options) => {
				if (action instanceof SubmenuItemAction && action.item.submenu === MenuId.CommandCenterCenter) {
					return instantiationService.createInstance(CommandCenterCenterViewItem, action, windowTitle, {
						...options,
						hoverDelegate
					});
				} else {
...
class CommandCenterCenterViewItem extends BaseActionViewItem {
	...
	constructor(
		...
		super(
			undefined,
			_submenu.actions.find(action => action.id === 'workbench.action.quickOpenWithModes') ?? _submenu.actions[0],
			options
		);
	...
	override render(container: HTMLElement): void {
		...
		const hover = this._store.add(
			this._hoverService.setupManagedHover(this._hoverDelegate, container, this.getTooltip())
		);
					...
					return this._instaService.createInstance(
						class CommandCenterQuickPickItem extends BaseActionViewItem {
							...
							override render(container: HTMLElement): void {
								...
								const aiCustomizationsDisabled =
									that._configurationService.getValue<boolean>('disableAICustomizations') === true ||
								...
								const isCompactMode =
									!forcedHidden &&
									(agentControlValue === true || agentControlValue === undefined || agentControlValue === 'compact');
								...
								this._store.add(
									that._editorGroupService.onDidChangeEditorPartOptions(({ newPartOptions, oldPartOptions }) => {
