...
function registerEditorMoveCopyCommand(): void {
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: (accessor, args) =>
			moveCopySelectedEditors(true, args as SelectedEditorsMoveCopyArguments | undefined, accessor),
		metadata: {
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		primary: 0,
		handler: (accessor, args) =>
			moveCopySelectedEditors(false, args as SelectedEditorsMoveCopyArguments | undefined, accessor),
		metadata: {
	...
	[
		...
		CommandsRegistry.registerCommand(id, function (accessor, ...args) {
			...
			if (resolvedContext.groupedEditors.length) {
				moveCopyEditorsToGroup(
					true,
					{ to, by: 'group' },
					resolvedContext.groupedEditors[0].group,
					resolvedContext.groupedEditors[0].editors,
					accessor
				);
			}
		});
	...
	function moveCopySelectedEditors(
		isMove: boolean,
		args: SelectedEditorsMoveCopyArguments = Object.create(null),
	...
	function moveCopyEditorsToGroup(
		isMove: boolean,
		args: SelectedEditorsMoveCopyArguments,
			...
			case 'previous':
				...
				if (!targetGroup) {
					const oppositeDirection =
						preferredSideBySideGroupDirection(configurationService) === GroupDirection.RIGHT
							? GroupDirection.LEFT
							: GroupDirection.UP;
...
function registerOpenEditorAPICommands(): void {
	...
	CommandsRegistry.registerCommand(
		...
		async function (
			...
			labelAndOrDescription?: string | { label: string; description: string },
			columnAndOptions?: [EditorGroupColumn?, ITextEditorOptions?],
			context?: IOpenEvent<unknown>
		) {
			const editorService = accessor.get(IEditorService);
	...
	CommandsRegistry.registerCommand(
		...
		async (
			...
			await editorService.openEditor(
				{ resource: URI.from(resource, true), options: { pinned: true, ...optionsArg, override: id } },
	...
	CommandsRegistry.registerCommand(
		'_workbench.changes',
		async (accessor: ServicesAccessor, title: string, resources: [UriComponents, UriComponents?, UriComponents?][]) => {
			const editorService = accessor.get(IEditorService);
	...
	CommandsRegistry.registerCommand(
		...
		async (accessor: ServicesAccessor, options: OpenMultiFileDiffEditorOptions) => {
			const editorService = accessor.get(IEditorService);
			...
			const revealResource =
				revealUri && resources ? resources.find(r => isEqual(r.modified.resource, revealUri)) : undefined;
...
function registerCloseEditorCommands() {
			...
			return Promise.all(
				resolvedContext.groupedEditors.map(async ({ group, editors }) => {
					const editorsToClose = group
						.getEditors(EditorsOrder.SEQUENTIAL, { excludeSticky: true })
	...
	KeybindingsRegistry.registerCommandAndKeybindingRule({
		...
		handler: async (accessor, ...args: unknown[]) => {
			...
			if (resolvedContext.groupedEditors.length) {
				...
				await group.closeEditors(
					{ direction: CloseDirection.RIGHT, except: editors[0], excludeSticky: true },
					{ preserveFocus: resolvedContext.preserveFocus }
				);
			}
		}
	...
	async function reopenEditorWith(
		accessor: ServicesAccessor,
		editorOverride: string | EditorResolution,
		...args: unknown[]
	) {
		const editorService = accessor.get(IEditorService);
		...
		for (const { group, editors } of resolvedContext.groupedEditors) {
			for (const editor of editors) {
				...
				telemetryService.publicLog2<WorkbenchEditorReopenEvent, WorkbenchEditorReopenClassification>(
...
function registerFocusEditorGroupWihoutWrapCommands(): void {
	...
	for (const command of commands) {
		CommandsRegistry.registerCommand(command.id, async (accessor: ServicesAccessor) => {
			...
			const group =
				editorGroupsService.findGroup({ direction: command.direction }, editorGroupsService.activeGroup, false) ??
