...
export class EditorAutoSave extends Disposable implements IWorkbenchContribution {
	...
	private registerListeners(): void {
		...
		this._register(
			this.filesConfigurationService.onDidChangeAutoSaveConfiguration(() => this.onDidChangeAutoSaveConfiguration())
		);
		...
		for (const resource of resources) {
			...
			// Waiting editors
				...
				if (
					...
					this.filesConfigurationService.getAutoSaveMode(editorResult.editor.editor, editorResult.reason).mode !==
						AutoSaveMode.OFF
				) {
				...
				if (
					(reason === SaveReason.WINDOW_CHANGE &&
						(autoSaveMode.mode === AutoSaveMode.ON_FOCUS_CHANGE ||
							autoSaveMode.mode === AutoSaveMode.ON_WINDOW_CHANGE)) ||
