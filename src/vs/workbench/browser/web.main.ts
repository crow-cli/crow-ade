...
export class BrowserMain extends Disposable {
	...
	async open(): Promise<IWorkbench> {
		...
		return instantiationService.invokeFunction(accessor => {
			...
			(globalThis as any).__crow_commandService = commandService;

			const editorService = accessor.get(IEditorService);
			(globalThis as any).__crow_editorService = editorService;
	...
	private async registerIndexedDBFileSystemProviders(
		...
		// Crow: All persistent file I/O is routed through the Tauri backend
		// (Rust `crow-workspace` + `crow-text`). The `vscode-userdata:`
		...
		const isTauri =
			...
			(globalThis as any).__CROW_TAURI__ === true;
		...
		if (isTauri) {
			...
			logService.info('[Crow] Registered TauriUserDataProvider for vscode-userdata:// scheme');

			const fileProvider = new TauriFileSystemProvider();
			fileService.registerProvider(Schemas.file, fileProvider);
			logService.info('[Crow] Registered TauriFileSystemProvider for file:// scheme');
			...
			logService.info('[Crow] Registered TauriFileSystemProvider for vscode-file:// scheme');
		} else {
			const userDataProvider = new InMemoryFileSystemProvider();
			fileService.registerProvider(Schemas.vscodeUserData, userDataProvider);
			this.registerDeveloperActions(userDataProvider);
			logService.warn('[Crow] Tauri backend not detected — user-data is in-memory only');
		}
	...
	private resolveWorkspace(): IAnyWorkspaceIdentifier {
		...
		console.log('[Crow] resolveWorkspace:', workspace, 'isFolderToOpen:', workspace && isFolderToOpen(workspace));
		...
		if (workspace && isFolderToOpen(workspace)) {
			...
			console.log('[Crow] Resolved single-folder workspace:', id);
			return id;
		}

		// Empty window workspace
		console.log('[Crow] No workspace resolved, using empty window');
