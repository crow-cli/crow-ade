...
import { CrowPart } from './parts/crow/crowPart.js';
...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private crowPartView!: ISerializableView;
	...
	private setSideBarPosition(position: Position): void {
		...
		const crowBar = this.getPart(Parts.CROW_PART);
	...
	private initLayoutState(lifecycleService: ILifecycleService, fileService: IFileService): void {
		...
		// Crow View to restore
		if (this.isVisible(Parts.CROW_PART)) {
			const viewContainerToRestore = this.storageService.get(
				CrowPart.activeViewSettingsKey,
				StorageScope.WORKSPACE,
				this.viewDescriptorService.getDefaultViewContainer(ViewContainerLocation.Crow)?.id
			);
			if (viewContainerToRestore) {
				this.state.initialization.views.containerToRestore.crow = viewContainerToRestore;
			} else {
				this.stateModel.setRuntimeValue(LayoutStateKeys.CROW_HIDDEN, true);
			}
		}
	...
	protected restoreParts(): void {
		...
		const restoreDefaultViewsPromise = (async () => {
			if (this.state.initialization.views.defaults?.length) {
				...
				// If we opened a view in the crow, stop any restore there
				if (locationsRestored[ViewContainerLocation.Crow]) {
					this.state.initialization.views.containerToRestore.crow =
						locationsRestored[ViewContainerLocation.Crow].id;
				}
		...
		layoutReadyPromises.push(
			(async () => {
				await restoreDefaultViewsPromise;
				if (!this.state.initialization.views.containerToRestore.crow) {
					return;
				}

				mark('code/willRestoreCrow');
	...
	focusPart(part: Parts, targetWindow: Window = mainWindow): void {
		...
		switch (part) {
			...
			case Parts.CROW_PART: {
				this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.Crow)?.focus();
				break;
			}
	...
	isVisible(part: Parts, targetWindow: Window = mainWindow): boolean {
		...
		switch (part) {
			...
			case Parts.CROW_PART:
				return !this.stateModel.getRuntimeValue(LayoutStateKeys.CROW_HIDDEN);
			case Parts.STATUSBAR_PART:
	...
	getMaximumEditorDimensions(container: HTMLElement): IDimension {
		...
		if (container === this.mainContainer) {
			...
			const takenWidth =
				...
				(this.isVisible(Parts.CROW_PART) ? this.crowPartView.minimumWidth : 0) +
	...
	toggleZenMode(skipLayout?: boolean, restoring = false): void {
		...
		if (this.isZenModeActive()) {
			...
			if (!restoring) {
				...
				zenModeExitInfo.wasVisible.crow = this.isVisible(Parts.CROW_PART);
		...
		// Zen Mode Inactive
			...
			if (zenModeExitInfo.wasVisible.crow) {
				this.setCrowHidden(false, true);
			}
	...
	protected createWorkbenchLayout(): void {
		...
		const crowPart = this.getPart(Parts.CROW_PART);
		const sideBar = this.getPart(Parts.SIDEBAR_PART);
		...
		const viewMap: Record<string, ISerializableView> = {
			...
			[Parts.CROW_PART]: this.crowPartView
		};
			...
			this._register(
				part.onDidVisibilityChange(visible => {
					if (!this.inMaximizedAuxiliaryBarTransition) {
						...
						} else if (part === crowPart) {
							this.setCrowHidden(!visible, true);
		...
		this._register(
			this.storageService.onWillSaveState(() => {
				...
				// Crow Size
				const crowSize = this.stateModel.getRuntimeValue(LayoutStateKeys.CROW_HIDDEN)
					? this.workbenchGrid.getViewCachedVisibleSize(this.crowPartView)
					: this.workbenchGrid.getViewSize(this.crowPartView).width;
				this.stateModel.setInitializationValue(LayoutStateKeys.CROW_SIZE, crowSize as number);
	...
	resizePart(part: Parts, sizeChangeWidth: number, sizeChangeHeight: number): void {
		...
		switch (part) {
			...
			case Parts.AUXILIARYBAR_PART:
				...
				break;
			case Parts.CROW_PART:
				viewSize = this.workbenchGrid.getViewSize(this.crowPartView);
				this.workbenchGrid.resizeView(this.crowPartView, {
					width: viewSize.width + sizeChangePxWidth,
					height: viewSize.height
				});
				break;
	...
	getLayoutClasses(): string[] {
		return coalesce([
			...
			!this.isVisible(Parts.CROW_PART) ? LayoutClasses.CROW_HIDDEN : undefined,
	...
	private adjustPartPositions(
		...
		const preMoveAuxiliaryBarSize = !this.isVisible(Parts.AUXILIARYBAR_PART)
			...
			: this.workbenchGrid.getViewSize(this.auxiliaryBarPartView).width;
		const preMoveCrowSize = !this.isVisible(Parts.CROW_PART)
			? Sizing.Invisible(
					this.workbenchGrid.getViewCachedVisibleSize(this.crowPartView) ??
						this.crowPartView.minimumWidth
				)
			: this.workbenchGrid.getViewSize(this.crowPartView).width;

		const focusedPart = [Parts.PANEL_PART, Parts.SIDEBAR_PART, Parts.AUXILIARYBAR_PART, Parts.CROW_PART].find(part =>
			this.hasFocus(part)
		...
		if (sideBarPosition === Position.LEFT) {
			...
			// Crow between editor and auxiliary bar
			this.workbenchGrid.moveView(
				this.crowPartView,
				preMoveCrowSize,
				this.editorPartView,
		...
		if (this.isVisible(Parts.CROW_PART)) {
			this.workbenchGrid.resizeView(this.crowPartView, {
				height: this.workbenchGrid.getViewSize(this.crowPartView).height,
				width: preMoveCrowSize as number
			});
		}
	}
	...
	private setCrowHidden(hidden: boolean, skipLayout?: boolean): void {
		this.stateModel.setRuntimeValue(LayoutStateKeys.CROW_HIDDEN, hidden);

		// Adjust CSS
		if (hidden) {
			this.mainContainer.classList.add(LayoutClasses.CROW_HIDDEN);
		} else {
			this.mainContainer.classList.remove(LayoutClasses.CROW_HIDDEN);
		}

		// Propagate to grid
		this.workbenchGrid.setViewVisible(this.crowPartView, !hidden);

		// If crow becomes hidden, also hide the current active pane composite if any
		if (hidden && this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.Crow)) {
			this.paneCompositeService.hideActivePaneComposite(ViewContainerLocation.Crow);
			this.focusPanelOrEditor();
		}

		// If crow becomes visible, show last active pane composite or default pane composite
		else if (!hidden && !this.paneCompositeService.getActivePaneComposite(ViewContainerLocation.Crow)) {
			let viewletToOpen: string | undefined = this.paneCompositeService.getLastActivePaneCompositeId(
				ViewContainerLocation.Crow
			);
			...
			if (!viewletToOpen || !this.hasViews(viewletToOpen)) {
				viewletToOpen = this.viewDescriptorService
					.getViewContainersByLocation(ViewContainerLocation.Crow)
					.find(viewContainer => this.hasViews(viewContainer.id))?.id;
			}

			if (viewletToOpen) {
				this.openViewContainer(ViewContainerLocation.Crow, viewletToOpen, !skipLayout);
			}
		}
	}
	...
	setPartHidden(hidden: boolean, part: Parts): void {
		switch (part) {
			...
			case Parts.CROW_PART:
				return this.setCrowHidden(hidden);
			case Parts.PANEL_PART:
	...
	setPanelPosition(position: Position): void {
		...
		const crowSize = this.workbenchGrid.getViewSize(this.crowPartView);
		...
		const crowVisible = this.isVisible(Parts.CROW_PART);
		...
		this.workbenchGrid.resizeView(this.crowPartView, crowSize);
		if (!crowVisible) {
			this.setCrowHidden(true);
		}
	...
	private arrangeEditorNodes(
		nodes: { editor: ISerializedNode; sideBar?: ISerializedNode; auxiliaryBar?: ISerializedNode; crow?: ISerializedNode },
		availableHeight: number,
		availableWidth: number
	): ISerializedNode {
		if (!nodes.sideBar && !nodes.auxiliaryBar && !nodes.crow) {
		...
		// Crow goes between editor and auxiliary bar
		if (nodes.crow) {
			if (this.stateModel.getRuntimeValue(LayoutStateKeys.SIDEBAR_POSITON) === Position.LEFT) {
				result.push(nodes.crow);
			} else {
				result.splice(0, 0, nodes.crow);
			}

			nodes.editor.size -= this.stateModel.getRuntimeValue(LayoutStateKeys.CROW_HIDDEN) ? 0 : nodes.crow.size;
		}
	...
	private arrangeMiddleSectionNodes(
		...
		const auxiliaryBarSize = this.stateModel.getRuntimeValue(LayoutStateKeys.AUXILIARYBAR_HIDDEN)
			? 0
			: nodes.auxiliaryBar.size;
		const crowSize = this.stateModel.getRuntimeValue(LayoutStateKeys.CROW_HIDDEN) ? 0 : nodes.crow.size;
		...
		if (!isHorizontal(panelPostion)) {
			...
			nodes.editor.size = availableWidth - activityBarSize - sideBarSize - panelSize - auxiliaryBarSize - crowSize;
			...
			if (sideBarPosition === Position.LEFT) {
				// [ActivityBar] [SideBar] [Editor] [Crow] [AuxiliaryBar]
				result.push(nodes.crow);
				result.push(nodes.auxiliaryBar);
				result.splice(0, 0, nodes.sideBar);
				result.splice(0, 0, nodes.activityBar);
			} else {
				// [AuxiliaryBar] [Crow] [Editor] [SideBar] [ActivityBar]
				result.splice(0, 0, nodes.crow);
				result.splice(0, 0, nodes.auxiliaryBar);
				result.push(nodes.sideBar);
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const crowSize = this.stateModel.getInitializationValue(LayoutStateKeys.CROW_SIZE);
		...
		const crowNode: ISerializedLeafNode = {
			type: 'leaf',
			data: { type: Parts.CROW_PART },
			size: crowSize,
			visible: this.isVisible(Parts.CROW_PART)
		};
...
const LayoutStateKeys = {
	...
	CROW_SIZE: new InitializationStateKey<number>('crow.size', StorageScope.PROFILE, StorageTarget.MACHINE, 350),
	...
	CROW_HIDDEN: new RuntimeStateKey<boolean>(
		'crow.hidden',
		StorageScope.WORKSPACE,
		StorageTarget.MACHINE,
		true
...
class LayoutStateModel extends Disposable {
	...
	private applyAuxiliaryBarMaximizedOverride(): void {
		...
		this.setRuntimeValue(LayoutStateKeys.CROW_HIDDEN, true);
