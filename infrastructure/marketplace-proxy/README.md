# Crow Marketplace Proxy

A Cloudflare Worker that gives Crow a Cursor-style extension
...
- Response header `x-crow-cache` is `edge | kv | miss`.
...
## Point Crow at the worker

After deploy, grab your worker URL (e.g.
`https://crow-marketplace-proxy.<account>.workers.dev`) and set:

- `crates/crow-extensions/src/marketplace.rs` →
