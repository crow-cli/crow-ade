		...
		const workers = Array.from({ length: 5 }, async () => {
			while (queue.length) {
				...
				try {
					...
					const edgeReq = new Request(`https://crow-cache/${encodeURIComponent(cacheKey)}`);
					await Promise.all([
...
async function handleVsGallery(url: URL, request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	if (edgeHit) {
		...
		if (respEtag) {
			...
			if (nm) {
				return withHeaders(nm, { 'x-crow-cache': 'edge-304' });
			}
		}
		return withHeaders(edgeHit, { 'x-crow-cache': 'edge' });
	}
	...
	if (kvHit) {
		...
		const nm = notModifiedResponse(request, respEtag);
		if (nm) {
			return withHeaders(nm, { 'x-crow-cache': 'kv-304' });
		}
...
function galleryResponse(body: string, cacheState: string, etag: string): Response {
	return new Response(body, {
		...
		headers: {
			...
			'access-control-allow-origin': '*',
			'access-control-expose-headers': 'etag, x-crow-cache, server-timing',
			vary: 'accept-encoding',
			etag,
			'x-crow-cache': cacheState
		}
	});
}
...
async function handleSearch(
	...
	if (edgeHit) {
		...
		if (etag) {
			...
			if (notModified) {
				return withHeaders(notModified, { 'x-crow-cache': 'edge-304' });
			}
		}
		return withHeaders(edgeHit, { 'x-crow-cache': 'edge' });
	}
	...
	if (kvHit) {
		...
		const notModified = notModifiedResponse(request, etag);
		if (notModified) {
			return withHeaders(notModified, { 'x-crow-cache': 'kv-304' });
		}
	...
	if (notModified) {
		...
		return withHeaders(notModified, { 'x-crow-cache': 'miss-304' });
	}
...
async function refreshSearch(
	...
	const pageSize = Math.min(Number(url.searchParams.get('pageSize') ?? url.searchParams.get('size')) || 50, 100);
	const offset = Math.max(Number(url.searchParams.get('offset')) || 0, 0);
	const body = await runSearch(query, pageSize, offset, shape, 'https://crow-cache');
	const etag = etagFor(body);
...
async function handleIcon(url: URL, _request: Request, _env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	const upstream = await fetch(target.toString(), {
		headers: { 'user-agent': 'crow-marketplace-proxy/1.0' }
	});
	if (!upstream.ok) {
...
async function handleAsset(url: URL, _request: Request, _env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	if (rest[0]?.startsWith('vsix-')) {
		...
		try {
			...
			const upstream = await fetch(vsixUrl, {
				headers: { 'user-agent': 'crow-marketplace-proxy/1.0', accept: '*/*' },
				redirect: 'follow'
			});
			if (!upstream.ok) {
	...
	const upstream = await fetch(upstreamUrl, {
		headers: {
			'user-agent': 'crow-marketplace-proxy/1.0',
			accept: '*/*'
		},
		redirect: 'follow'
	});
...
async function handleDownload(url: URL, request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	if (edgeHit && edgeHit.body) {
		return range ? partialFrom(edgeHit, range, 'edge') : withHeaders(edgeHit, { 'x-crow-cache': 'edge' });
	}
...
async function partialFrom(full: Response, range: string, cacheState: string): Promise<Response> {
	...
	if (!match) {
		return withHeaders(full, { 'x-crow-cache': cacheState });
	}
	...
	headers.set('x-crow-cache', cacheState);
