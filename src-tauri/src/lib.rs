...
use commands::db_state::CrowDbState;
...
fn build_menu(app: &tauri::AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let crow_menu = SubmenuBuilder::with_id(app, "crow_menu", "Crow")
        .item(&PredefinedMenuItem::about(app, Some("About Crow"), None)?)
        .separator()
...
pub fn run() {
    tauri::Builder::default()
        ...
        .manage(Arc::new(crow_extension_api::CommandRegistry::new()))
        ...
        .register_asynchronous_uri_scheme_protocol("crow-asset", |_ctx, request, responder| {
        ...
        .setup(|app| {
            ...
            let db_path = app_data.join("crow_storage.db");
            ...
            let crow_db_path = app_data.join("crow_state.db");
            let crow_db = crow_db::Database::open(&crow_db_path)
                .expect("failed to initialize crow-db state database");
            app.manage(Arc::new(CrowDbState::new(crow_db)));
        ...
        .on_menu_event(|app, event| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                ...
                let _ = window.eval(format!(
                    "window.dispatchEvent(new CustomEvent('crow-native-menu', {{ detail: '{escaped}' }}))"
                ));
            }
        })
		...
		commands::textmate_tokenize_line_binary,
            ...
            // Marketplace & contributions (crow-extensions)
