...
// ---------------------------------------------------------------------------
// crow-dap integration
// ---------------------------------------------------------------------------

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct DapLaunchConfigResponse {
    pub configs: Vec<crow_dap::LaunchConfig>,
    pub compounds: Vec<crow_dap::CompoundLaunchConfig>,
}
...
pub fn dap_get_launch_configs(workspace: String) -> Result<DapLaunchConfigResponse, String> {
    ...
    let (configs, compounds) =
        crow_dap::parse_launch_json(&launch_path).map_err(|e| e.to_string())?;
...
pub fn dap_get_adapter_registry() -> Result<Vec<DebugAdapterInfo>, String> {
    let registry = crow_dap::DebugAdapterRegistry::with_builtins();
    let infos = registry
        .registered_types()
        .into_iter()
        .filter_map(|t| {
...
// ---------------------------------------------------------------------------
// High-level DAP client (crow-dap DebugClient)
// ---------------------------------------------------------------------------

pub struct DapClientStore {
    clients: AsyncMutex<HashMap<u32, Arc<crow_dap::DebugClient>>>,
...
pub struct DapStartResult {
    pub adapter_id: u32,
    pub capabilities: crow_dap::Capabilities,
}

/// Launch a debug session via `crow-dap`. Resolves `type_name` against the
...
pub async fn dap_start_adapter(
    ...
    let client = crow_dap::DebugClient::launch(&command_line, &config)
        .await
...
pub async fn dap_send_request(
    ...
    match command.as_str() {
        ...
        "setBreakpoints" => {
            ...
            let breakpoints: Vec<crow_dap::SourceBreakpoint> = args
                .get("breakpoints")
                .cloned()
