...
// ── Auto-detection & tasks.json parsing via crow-tasks ─────────────
...
impl DetectedTask {
    fn from_crate_task(t: &crow_tasks::Task) -> Self {
        Self {
            label: t.name.clone(),
...
impl TaskDefinition {
    fn from_crate_task(t: &crow_tasks::Task) -> Self {
        let group = match t.group {
            crow_tasks::TaskGroup::None => None,
...
pub fn tasks_detect(workspace: String) -> Result<Vec<DetectedTask>, String> {
    ...
    for result in [
        crow_tasks::detect_npm_tasks(root),
        crow_tasks::detect_cargo_tasks(root),
        crow_tasks::detect_make_tasks(root),
    ] {
        match result {
...
pub fn tasks_parse_config(workspace: String) -> Result<Vec<TaskDefinition>, String> {
    ...
    let tasks = crow_tasks::parse_tasks_json(&tasks_path).map_err(|e| e.to_string())?;
