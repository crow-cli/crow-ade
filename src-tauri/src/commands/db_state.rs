...
use serde::Serialize;
use crow_db::Database;
use tauri::State;

pub struct CrowDbState {
    db: Mutex<Database>,
}
...
pub fn db_get_recent_files(
    state: State<'_, Arc<CrowDbState>>,
    limit: u32,
) -> Result<Vec<RecentFileEntry>, String> {
    let db = state.db.lock().map_err(|e| e.to_string())?;
    crow_db::recent_files(&db, limit as usize)
        .map(|v| {
            v.into_iter()
...
pub fn db_get_recent_workspaces(
    ...
    let db = state.db.lock().map_err(|e| e.to_string())?;
    crow_db::recent_workspaces(&db, limit as usize)
        .map(|v| {
            v.into_iter()
...
pub fn db_save_workspace_state(
    ...
    crow_db::set_workspace_state(&db, &workspace, &key, &json_val).map_err(|e| e.to_string())
}
...
pub fn db_get_workspace_state(
    ...
    let db = state.db.lock().map_err(|e| e.to_string())?;
    crow_db::get_workspace_state(&db, &workspace, &key)
