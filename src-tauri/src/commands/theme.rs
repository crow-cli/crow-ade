...
use serde::Serialize;
use crow_theme::theme::{Theme, ThemeKind};
use crow_theme::theme_resolver::{ThemeRegistry, UiTheme};
use crow_theme::token_color::FontStyle;
...
fn workbench_colors_to_map(wb: &crow_theme::WorkbenchColors) -> HashMap<String, String> {
...
fn convert_token_rule(rule: &crow_theme::TokenColorRule) -> TokenColorRule {
    TokenColorRule {
        scope: rule.scope.clone(),
        settings: TokenColorSettings {
            foreground: rule.foreground.map(crow_theme::Color::to_hex),
