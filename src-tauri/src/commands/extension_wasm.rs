...
impl TsServerProcess {
    ...
    fn spawn(workspace_folders: &[String]) -> Option<Self> {
        ...
        let node_bin = std::env::var("CROW_NODE_BINARY").unwrap_or_else(|_| "node".to_string());
...
use wit_bindings::crow::extension::common_types as wit_types;
use wit_bindings::CrowExtension;
...
impl wit_bindings::crow::extension::host_api::Host for WasmHostState {
...
impl wit_bindings::crow::extension::common_types::Host for WasmHostState {}

// ---------------------------------------------------------------------------
// tsserver dispatch — called from execute_command("__crow.tsserver", ...)
// ---------------------------------------------------------------------------
...
impl WasmHostState {
    ...
    /// Handle __crow.lsp commands. Payload format:
...
impl WasmExtensionRuntime {
    pub fn new() -> Result<Self> {
        ...
        CrowExtension::add_to_linker::<_, wasmtime::component::HasSelf<_>>(&mut linker, |x| x)
    ...
    pub fn load_extension(&self, manifest: &ExtensionManifest) -> Result<(), String> {
        ...
        let bindings = CrowExtension::instantiate(&mut store, &component, &guard.linker)
            .map_err(|e| format!("failed to instantiate {}: {e}", manifest.id))?;

        bindings
            .crow_extension_extension_api()
...
pub async fn wasm_get_extension_metadata(
    ...
    let api = ext.bindings.crow_extension_extension_api();
