...
/// Works for both Node (package.json) and WASM (crow.toml) extensions.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
struct CrowTomlManifest {
    extension: CrowTomlExtension,
    activation: Option<CrowTomlActivation>,
    contributes: Option<CrowTomlContributes>,
}

#[allow(dead_code)]
#[derive(Debug, Deserialize)]
struct CrowTomlExtension {
    id: String,
    name: String,
    version: String,
    #[serde(default)]
...
#[derive(Debug, Deserialize)]
struct CrowTomlActivation {
    #[serde(default)]
    events: Vec<String>,
}

#[derive(Debug, Deserialize)]
struct CrowTomlContributes {
    #[serde(default)]
    languages: Vec<String>,
    #[serde(default)]
    commands: Vec<CrowTomlCommand>,
}

#[allow(dead_code)]
#[derive(Debug, Deserialize)]
struct CrowTomlCommand {
    id: String,
    title: String,
}
...
pub fn user_extensions_dir() -> PathBuf {
    crow_extensions::paths::user_extensions_dir()
}

pub fn global_storage_dir() -> PathBuf {
    crow_extensions::paths::global_storage_dir()
}

fn user_data_dir() -> PathBuf {
    crow_extensions::paths::user_data_dir()
}
...
pub fn bundled_node_candidates(app: &AppHandle) -> Vec<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CROW_NODE_BINARY") {
...
pub fn resolve_node_runtime(app: &AppHandle) -> Result<ResolvedNode, String> {
    ...
    Err("Node runtime not found. Bundle Node with Crow or install Node.js (>=18).".into())
}
...
fn is_version_greater(a: &str, b: &str) -> bool {
    crow_extensions::manifest::is_version_greater(a, b)
}
...
pub fn read_extension_manifest(
    ...
    let raw = crow_extensions::encoding::read_manifest_file(&pkg_path)
...
pub fn scan_extensions(app: &AppHandle, paths: &[PathBuf]) -> Vec<ExtensionManifest> {
    let mut disable_ids: HashSet<String> = std::env::var("CROW_DISABLE_EXTENSION_IDS")
    ...
    for search_path in paths {
        ...
        for entry in entries.flatten() {
            ...
            let manifest = if ext_dir.join("crow.toml").exists() {
...
pub fn build_extension_descriptions(manifests: &[ExtensionManifest]) -> Vec<ExtensionDescription> {
    manifests
        ...
        .map(|m| {
            ...
            let package_json = crow_extensions::encoding::read_manifest_file(&pkg_path)
                .ok()
    ...
    ExtensionHostInitData {
        ...
        environment: InitDataEnvironment {
            ...
            app_root: std::env::current_dir()
                ...
                .to_string_lossy()
                .to_string(),
            app_name: "Crow".to_string(),
            app_host: "desktop".to_string(),
            app_uri_scheme: "crow".to_string(),
...
pub async fn extension_platform_bootstrap(
    ...
    if !wasm_manifests.is_empty() {
        ...
        std::thread::spawn(move || {
            ...
            let _ = app_handle.emit("crow-wasm-extensions-ready", count);
        });
    }
