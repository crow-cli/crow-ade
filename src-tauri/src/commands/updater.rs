...
//! and listens to the `crow://update/state-change` event to mirror
//! `onStateChange`.

use std::path::PathBuf;
use std::sync::{Arc, OnceLock};

use crow_update::{State, UpdateConfig, UpdateManager, UpdateObserver, UpdateResult, UpdateType};
use tauri::{AppHandle, Emitter, Manager};

/// Tauri event name carrying the latest [`State`] payload.
pub const STATE_EVENT: &str = "crow://update/state-change";
...
pub fn update_quit_and_install(app: AppHandle) -> Result<(), String> {
    ...
    crow_update::install::relaunch(&install_root).map_err(|e| e.to_string())?;
    app.exit(0);
    Ok(())
}
