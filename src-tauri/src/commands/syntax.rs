use serde::Serialize;
use crow_syntax::highlight::{HighlightConfig, Highlighter, SyntaxHighlighter, TokenScope};
use crow_syntax::language::{builtin_language_configurations, LanguageConfiguration};
use std::path::Path;
...
pub fn textmate_tokenize_lines(
    ...
    let grammar = crow_syntax::textmate::TextMateGrammar::from_json(&grammar_json)
        .map_err(|e| format!("parse grammar: {e}"))?;
    let tokenizer = crow_syntax::textmate::TextMateTokenizer::new(&grammar);
    let mut state = crow_syntax::textmate::TokenizerState::default();
