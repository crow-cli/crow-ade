use serde::Serialize;
use crow_text::diff::{compute_line_diff, LineDiff};
use crow_text::encoding::detect_encoding;
use crow_text::Buffer;
use crow_text::{detect_line_ending, line_ending_label, normalize_line_endings, LineEnding};
use std::io::Read;
...
pub fn file_summary(path: String) -> Result<FileSummary, String> {
    ...
    let has_bom = matches!(encoding, crow_text::encoding::Encoding::Utf8Bom);
    ...
    for line_idx in 0..buffer.len_lines() {
        for word_info in buffer.words_at(line_idx) {
            if word_info.word_type == crow_text::WordType::Word {
                word_count += 1;
            }
        }
    }
...
pub fn get_word_boundaries(line: String, column: usize) -> Result<WordBoundary, String> {
    ...
    let pos = crow_text::Position::new(0, column as u32);
