...
fn spawn_host_process(
    ...
    let init_data_file = std::env::temp_dir().join(format!("crow-init-{}.json", &session_id));
    ...
    child_cmd
        ...
        .arg(&server_js)
        .env("CROW_EXTENSIONS_DIR", &user_ext_dir)
        .env("CROW_BUILTIN_EXTENSIONS_DIR", &builtin_ext_dir)
        .env("CROW_GLOBAL_STORAGE_DIR", &global_store_dir)
        .env("CROW_EXTENSION_SEARCH_PATHS", &search_paths_json)
        .env("CROW_INIT_DATA_FILE", &init_data_file)
        .env("CROW_SESSION_ID", &session_id)
