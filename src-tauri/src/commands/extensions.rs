...
use serde::Serialize;
use crow_extensions::contributions::{parse_contributions, ContributionPoint};
use crow_extensions::installer::{
    install_from_vsix as crate_install_from_vsix, uninstall as crate_uninstall,
};
use crow_extensions::manifest::sanitize_ext_id;
use crow_extensions::marketplace::MarketplaceClient;
use crow_extensions::paths::user_extensions_dir;
use crow_extensions::vsix::{install_package, unpack_vsix, validate_vsix};
use std::fs;
use std::path::Path;
use std::sync::Arc;
use tauri::AppHandle;
...
fn to_installed(
    manifest: &crow_extensions::manifest::ExtensionManifest,
    path: &Path,
) -> InstalledExtension {
    InstalledExtension {
...
pub async fn install_extension_from_url(url: String) -> Result<InstalledExtension, String> {
    ...
    let tmp_path = std::env::temp_dir().join(format!("crow-{}.vsix", uuid::Uuid::new_v4()));
