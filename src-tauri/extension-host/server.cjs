...
const CROW_DATA_DIR = path.join(os.homedir(), '.crow');
const EXTENSIONS_DIR = process.env.CROW_EXTENSIONS_DIR || path.join(CROW_DATA_DIR, 'extensions');
const USER_DATA_DIR = path.join(CROW_DATA_DIR, 'data');
const GLOBAL_STORAGE_DIR = process.env.CROW_GLOBAL_STORAGE_DIR || path.join(USER_DATA_DIR, 'User', 'globalStorage');
...
try {
  if (process.env.CROW_INIT_DATA_FILE) {
    const raw = fs.readFileSync(process.env.CROW_INIT_DATA_FILE, 'utf8');
    rustInitData = JSON.parse(raw);
    log(`received Rust-generated init data with ${(rustInitData.extensions || []).length} extensions`);
    try { fs.unlinkSync(process.env.CROW_INIT_DATA_FILE); } catch {}
  } else if (process.env.CROW_INIT_DATA) {
    rustInitData = JSON.parse(process.env.CROW_INIT_DATA);
    log(`received Rust-generated init data with ${(rustInitData.extensions || []).length} extensions`);
  }
} catch (e) {
...
function getExtensionSearchPaths() {
  if (process.env.CROW_EXTENSION_SEARCH_PATHS) {
    try {
      const parsed = JSON.parse(process.env.CROW_EXTENSION_SEARCH_PATHS);
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.filter((p) => typeof p === 'string' && p.length > 0);
      }
    } catch (e) {
      log(`bad CROW_EXTENSION_SEARCH_PATHS: ${e.message}`);
    }
  }

  const builtinExt = process.env.CROW_BUILTIN_EXTENSIONS_DIR;
  const candidates = [
    EXTENSIONS_DIR,
    builtinExt,
...
class ExtensionHostManager {
  ...
  getOrCreateSharedProcess(initData) {
    ...
    const initDataFile = path.join(os.tmpdir(), `crow-host-${reconnectionToken}.json`);
    try {
    ...
    const child = cp.fork(hostPath, ['--type=extensionHost'], {
      ...
      env: {
        ...
        CROW_EXTENSION_HOST: 'true',
        CROW_INIT_DATA_FILE: initDataFile,
      },
    });
...
class ClientConnection {
  ...
  _setupIPC() {
    this._messageListener = (msg) => {
      ...
      if (msg && msg.type === 'crow:host-event') {
      ...
      if (msg && msg.type === 'crow:host-reply') {
...
function scanExtensionsFallback(searchPaths) {
  const disableIds = new Set(
    (process.env.CROW_DISABLE_EXTENSION_IDS || 'ms-python.vscode-pylance')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
  );
...
async function main() {
  ...
  const server = http.createServer((_req, res) => {
    ...
    res.end(
      JSON.stringify({
        ...
        pid: process.pid,
        connectionToken: hostManager.connectionToken,
        sessionId: process.env.CROW_SESSION_ID || null,
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    log(`listening on 127.0.0.1:${port} (session=${process.env.CROW_SESSION_ID || 'unknown'})`);
  });
