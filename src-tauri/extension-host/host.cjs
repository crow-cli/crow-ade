...
class ExtensionHost extends EventEmitter {
  ...
  _registerBuiltinCommands() {
    this._commands.set('crow.extHost.ping', () => 'pong');
    this._commands.set('setContext', () => undefined);
    this._commands.set('crow.extHost.listLoaded', () => {
      const list = [];
...
function createVscodeShim() {
  ...
  const window = {
    ...
    createStatusBarItem(alignmentOrId, priorityOrAlignment, priorityArg) {
      ...
      if (typeof alignmentOrId === 'string') {
        ...
        itemId = `crow-statusbar-${++host._reqId}`;
    ...
    createTerminal(nameOrOptions, shellPath, shellArgs) {
      const termId = `crow-term-${++host._reqId}`;
      let opts = {};
    ...
    createQuickPick() {
      ...
      let _selectedItems = [];
      let _buttons = [];
      const qpId = `crow-qp-${++host._reqId}`;
      return {
    ...
    createInputBox() {
      ...
      const ibId = `crow-ib-${++host._reqId}`;
      return {
        get value() { return _value; },
    ...
    createWebviewPanel: (viewType, title, showOptions, options) => {
      const panelId = `crow-webview-${++host._reqId}`;
  ...
  const tasks = (() => {
    ...
    return {
      ...
      executeTask: (task) => {
        const execId = `crow-taskexec-${++host._reqId}`;
        host.emit('event', {
          type: 'executeTask',
          id: execId,
          name: task.name,
...
function installVscodeShim() {
  ...
  Module._resolveFilename = function (request, parent, isMain, options) {
    if (request === 'vscode') return '__crow_vscode_shim__';
    return original.call(this, request, parent, isMain, options);
  };
  require.cache['__crow_vscode_shim__'] = {
    id: '__crow_vscode_shim__',
    filename: '__crow_vscode_shim__',
    loaded: true,
    exports: null,
  };
  ...
  try {
    ...
    fs.writeFileSync(path.join(vscDir, 'index.cjs'), "module.exports = globalThis.__crow_vscode_shim__ || require('__crow_vscode_shim__');");
  } catch {}
}
...
globalThis.__crow_vscode_shim__ = _vscodeShim;
require.cache['__crow_vscode_shim__'].exports = _vscodeShim;

try {
  const { register } = require('module');
  if (typeof register === 'function') {
    const hostCjsPath = path.resolve(__dirname, 'host.cjs');
    const shimExports = require.cache['__crow_vscode_shim__']?.exports;
    ...
    const esmLines = [
      ...
      `const _shim = globalThis.__crow_vscode_shim__ || _require('__crow_vscode_shim__');`,
...
if (process.env.CROW_EXTENSION_HOST === 'true' && process.send) {
  const host = hostInstance;
  host.initialize();

  let initData = null;
  try {
    if (process.env.CROW_INIT_DATA_FILE) {
      const raw = require('fs').readFileSync(process.env.CROW_INIT_DATA_FILE, 'utf8');
      initData = JSON.parse(raw);
      try { require('fs').unlinkSync(process.env.CROW_INIT_DATA_FILE); } catch {}
    } else if (process.env.CROW_INIT_DATA) {
      initData = JSON.parse(process.env.CROW_INIT_DATA);
    }
  } catch (e) {
  ...
  host.on('event', (event) => {
    if (process.send) {
      process.send({ type: 'crow:host-event', event });
    }
  });
  ...
  process.on('message', (msg) => {
    ...
    if (reply && typeof reply.then === 'function') {
      reply.then((r) => {
        if (r && process.send) process.send({ type: 'crow:host-reply', reply: r });
      }).catch(() => {});
    } else if (reply && process.send) {
      process.send({ type: 'crow:host-reply', reply });
    }
  });
